# Debugging Runtime Specification

## Overview
The `Debugging` runtime specification governs real-time execution state, queue metrics, worker thread allocations, and memory checkpoints for Debugging in Vibe Agents.
...
- Monitor dynamic process lifecycle, state transitions, and worker queue depths for `Debugging`.
...
```mermaid
flowchart TD
    API[Express.js / Socket.IO API Engine] --> Manager[Runtime Manager: Debugging]
...
```typescript
interface DebuggingRuntimeState {
  id: string;
  tenantId: string;
