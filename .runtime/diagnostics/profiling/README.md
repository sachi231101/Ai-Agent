# Profiling Runtime Specification

## Overview
The `Profiling` runtime specification governs real-time execution state, queue metrics, worker thread allocations, and memory checkpoints for Profiling in Vibe Agents.
...
- Monitor dynamic process lifecycle, state transitions, and worker queue depths for `Profiling`.
...
```mermaid
flowchart TD
    API[Express.js / Socket.IO API Engine] --> Manager[Runtime Manager: Profiling]
...
```typescript
interface ProfilingRuntimeState {
  id: string;
  tenantId: string;
