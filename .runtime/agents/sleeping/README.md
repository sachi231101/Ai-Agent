# Sleeping Runtime Specification

## Overview
The `Sleeping` runtime specification governs real-time execution state, queue metrics, worker thread allocations, and memory checkpoints for Sleeping in Vibe Agents.
...
- Monitor dynamic process lifecycle, state transitions, and worker queue depths for `Sleeping`.
...
```mermaid
flowchart TD
    API[Express.js / Socket.IO API Engine] --> Manager[Runtime Manager: Sleeping]
...
```typescript
interface SleepingRuntimeState {
  id: string;
  tenantId: string;
