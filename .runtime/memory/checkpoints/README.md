# Checkpoints Runtime Specification

## Overview
The `Checkpoints` runtime specification governs real-time execution state, queue metrics, worker thread allocations, and memory checkpoints for Checkpoints in Vibe Agents.
...
- Monitor dynamic process lifecycle, state transitions, and worker queue depths for `Checkpoints`.
...
```mermaid
flowchart TD
    API[Express.js / Socket.IO API Engine] --> Manager[Runtime Manager: Checkpoints]
...
```typescript
interface CheckpointsRuntimeState {
  id: string;
  tenantId: string;
