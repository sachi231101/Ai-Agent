# Outgoing Runtime Specification

## Overview
The `Outgoing` runtime specification governs real-time execution state, queue metrics, worker thread allocations, and memory checkpoints for Outgoing in Vibe Agents.
...
- Monitor dynamic process lifecycle, state transitions, and worker queue depths for `Outgoing`.
...
```mermaid
flowchart TD
    API[Express.js / Socket.IO API Engine] --> Manager[Runtime Manager: Outgoing]
...
```typescript
interface OutgoingRuntimeState {
  id: string;
  tenantId: string;
