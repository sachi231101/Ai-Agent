# Incoming Runtime Specification

## Overview
The `Incoming` runtime specification governs real-time execution state, queue metrics, worker thread allocations, and memory checkpoints for Incoming in Vibe Agents.
...
- Monitor dynamic process lifecycle, state transitions, and worker queue depths for `Incoming`.
...
```mermaid
flowchart TD
    API[Express.js / Socket.IO API Engine] --> Manager[Runtime Manager: Incoming]
...
```typescript
interface IncomingRuntimeState {
  id: string;
  tenantId: string;
