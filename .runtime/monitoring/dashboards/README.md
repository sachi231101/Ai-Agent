# Dashboards Runtime Specification

## Overview
The `Dashboards` runtime specification governs real-time execution state, queue metrics, worker thread allocations, and memory checkpoints for Dashboards in Vibe Agents.
...
- Monitor dynamic process lifecycle, state transitions, and worker queue depths for `Dashboards`.
...
```mermaid
flowchart TD
    API[Express.js / Socket.IO API Engine] --> Manager[Runtime Manager: Dashboards]
...
```typescript
interface DashboardsRuntimeState {
  id: string;
  tenantId: string;
