# Performance Runtime Specification

## Overview
The `Performance` runtime specification governs real-time execution state, queue metrics, worker thread allocations, and memory checkpoints for Performance in Vibe Agents.
...
- Monitor dynamic process lifecycle, state transitions, and worker queue depths for `Performance`.
...
```mermaid
flowchart TD
    API[Express.js / Socket.IO API Engine] --> Manager[Runtime Manager: Performance]
...
```typescript
interface PerformanceRuntimeState {
  id: string;
  tenantId: string;
