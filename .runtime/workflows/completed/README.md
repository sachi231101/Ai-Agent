# Completed Runtime Specification

## Overview
The `Completed` runtime specification governs real-time execution state, queue metrics, worker thread allocations, and memory checkpoints for Completed in Vibe Agents.
...
- Monitor dynamic process lifecycle, state transitions, and worker queue depths for `Completed`.
...
```mermaid
flowchart TD
    API[Express.js / Socket.IO API Engine] --> Manager[Runtime Manager: Completed]
...
```typescript
interface CompletedRuntimeState {
  id: string;
  tenantId: string;
