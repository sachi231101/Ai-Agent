...
## Overview
The `Active` runtime specification governs real-time execution state, queue metrics, worker thread allocations, and memory checkpoints for Active in Vibe Agents.
...
- Monitor dynamic process lifecycle, state transitions, and worker queue depths for `Active`.
...
```mermaid
flowchart TD
    API[Express.js / Socket.IO API Engine] --> Manager[Runtime Manager: Active]
