# Archived Runtime Specification

## Overview
The `Archived` runtime specification governs real-time execution state, queue metrics, worker thread allocations, and memory checkpoints for Archived in Vibe Agents.
...
- Monitor dynamic process lifecycle, state transitions, and worker queue depths for `Archived`.
...
```mermaid
flowchart TD
    API[Express.js / Socket.IO API Engine] --> Manager[Runtime Manager: Archived]
...
```typescript
interface ArchivedRuntimeState {
  id: string;
  tenantId: string;
