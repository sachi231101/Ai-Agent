# Scheduled Runtime Specification

## Overview
The `Scheduled` runtime specification governs real-time execution state, queue metrics, worker thread allocations, and memory checkpoints for Scheduled in Vibe Agents.
...
- Monitor dynamic process lifecycle, state transitions, and worker queue depths for `Scheduled`.
...
```mermaid
flowchart TD
    API[Express.js / Socket.IO API Engine] --> Manager[Runtime Manager: Scheduled]
...
```typescript
interface ScheduledRuntimeState {
  id: string;
  tenantId: string;
