# Cancelled Runtime Specification

## Overview
The `Cancelled` runtime specification governs real-time execution state, queue metrics, worker thread allocations, and memory checkpoints for Cancelled in Vibe Agents.
...
- Monitor dynamic process lifecycle, state transitions, and worker queue depths for `Cancelled`.
...
```mermaid
flowchart TD
    API[Express.js / Socket.IO API Engine] --> Manager[Runtime Manager: Cancelled]
...
```typescript
interface CancelledRuntimeState {
  id: string;
  tenantId: string;
