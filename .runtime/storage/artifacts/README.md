...
## Overview
The `Artifacts` runtime specification governs real-time execution state, queue metrics, worker thread allocations, and memory checkpoints for Artifacts in Vibe Agents.
...
- Monitor dynamic process lifecycle, state transitions, and worker queue depths for `Artifacts`.
...
```mermaid
flowchart TD
    API[Express.js / Socket.IO API Engine] --> Manager[Runtime Manager: Artifacts]
...
```typescript
interface ArtifactsRuntimeState {
  id: string;
  tenantId: string;
