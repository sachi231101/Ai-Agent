# Recurring Runtime Specification

## Overview
The `Recurring` runtime specification governs real-time execution state, queue metrics, worker thread allocations, and memory checkpoints for Recurring in Vibe Agents.
...
- Monitor dynamic process lifecycle, state transitions, and worker queue depths for `Recurring`.
...
```mermaid
flowchart TD
    API[Express.js / Socket.IO API Engine] --> Manager[Runtime Manager: Recurring]
...
```typescript
interface RecurringRuntimeState {
  id: string;
  tenantId: string;
