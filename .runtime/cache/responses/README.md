# Responses Runtime Specification

## Overview
The `Responses` runtime specification governs real-time execution state, queue metrics, worker thread allocations, and memory checkpoints for Responses in Vibe Agents.
...
- Monitor dynamic process lifecycle, state transitions, and worker queue depths for `Responses`.
...
```mermaid
flowchart TD
    API[Express.js / Socket.IO API Engine] --> Manager[Runtime Manager: Responses]
...
```typescript
interface ResponsesRuntimeState {
  id: string;
  tenantId: string;
