# Terminals Runtime Specification

## Overview
The `Terminals` runtime specification governs real-time execution state, queue metrics, worker thread allocations, and memory checkpoints for Terminals in Vibe Agents.
...
- Monitor dynamic process lifecycle, state transitions, and worker queue depths for `Terminals`.
...
```mermaid
flowchart TD
    API[Express.js / Socket.IO API Engine] --> Manager[Runtime Manager: Terminals]
...
```typescript
interface TerminalsRuntimeState {
  id: string;
  tenantId: string;
