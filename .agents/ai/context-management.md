...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `context-management`.
...
flowchart TD
    ...
    Gateway --> Service[ContextManagement Service]
...
## Internal Components
- **`ContextManagementController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`ContextManagementService`**: Core business domain engine executing operational logic.
- **`ContextManagementRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useContextManagementStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/ai/context-management/execute
interface ExecuteContextManagementRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteContextManagementResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `ai:context-management:update`
- **Listen Event**: `ai:context-management:subscribe`
...
```typescript
// Operational Algorithm for Context Management Domain Processing
export async function executeContextManagementTask(
  ctx: RequestContext,
  payload: ExecuteContextManagementRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteContextManagementResponseDTO> {
  ...
  const response: ExecuteContextManagementResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `CONTEXT-MANAGEMENT_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `CONTEXT-MANAGEMENT_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `CONTEXT-MANAGEMENT_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
