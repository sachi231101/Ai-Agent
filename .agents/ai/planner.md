...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `planner`.
...
flowchart TD
    ...
    Gateway --> Service[Planner Service]
...
## Internal Components
- **`PlannerController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`PlannerService`**: Core business domain engine executing operational logic.
- **`PlannerRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`usePlannerStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/ai/planner/execute
interface ExecutePlannerRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecutePlannerResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `ai:planner:update`
- **Listen Event**: `ai:planner:subscribe`
...
```typescript
// Operational Algorithm for Planner Domain Processing
export async function executePlannerTask(
  ctx: RequestContext,
  payload: ExecutePlannerRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecutePlannerResponseDTO> {
  ...
  const response: ExecutePlannerResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `PLANNER_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `PLANNER_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `PLANNER_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
