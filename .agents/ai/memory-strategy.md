...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `memory-strategy`.
...
flowchart TD
    ...
    Gateway --> Service[MemoryStrategy Service]
...
## Internal Components
- **`MemoryStrategyController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`MemoryStrategyService`**: Core business domain engine executing operational logic.
- **`MemoryStrategyRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useMemoryStrategyStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/ai/memory-strategy/execute
interface ExecuteMemoryStrategyRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteMemoryStrategyResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `ai:memory-strategy:update`
- **Listen Event**: `ai:memory-strategy:subscribe`
...
```typescript
// Operational Algorithm for Memory Strategy Domain Processing
export async function executeMemoryStrategyTask(
  ctx: RequestContext,
  payload: ExecuteMemoryStrategyRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteMemoryStrategyResponseDTO> {
  ...
  const response: ExecuteMemoryStrategyResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `MEMORY-STRATEGY_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `MEMORY-STRATEGY_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `MEMORY-STRATEGY_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
