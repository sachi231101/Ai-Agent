...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `hallucination-prevention`.
...
flowchart TD
    ...
    Gateway --> Service[HallucinationPrevention Service]
...
## Internal Components
- **`HallucinationPreventionController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`HallucinationPreventionService`**: Core business domain engine executing operational logic.
- **`HallucinationPreventionRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useHallucinationPreventionStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/ai/hallucination-prevention/execute
interface ExecuteHallucinationPreventionRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteHallucinationPreventionResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `ai:hallucination-prevention:update`
- **Listen Event**: `ai:hallucination-prevention:subscribe`
...
```typescript
// Operational Algorithm for Hallucination Prevention Domain Processing
export async function executeHallucinationPreventionTask(
  ctx: RequestContext,
  payload: ExecuteHallucinationPreventionRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteHallucinationPreventionResponseDTO> {
  ...
  const response: ExecuteHallucinationPreventionResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `HALLUCINATION-PREVENTION_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `HALLUCINATION-PREVENTION_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `HALLUCINATION-PREVENTION_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
...
## Related Files
- [ai-overview.md](file:///C:/Users/admin/OneDrive/Desktop/emergent/Ai Agent/.agents/ai/ai-overview.md)
- [model-routing.md](file:///C:/Users/admin/OneDrive/Desktop/emergent/Ai Agent/.agents/ai/model-routing.md)
- [prompt-engineering.md](file:///C:/Users/admin/OneDrive/Desktop/emergent/Ai Agent/.agents/ai/prompt-engineering.md)
- [multi-agent.md](file:///C:/Users/admin/OneDrive/Desktop/emergent/Ai Agent/.agents/ai/multi-agent.md)
