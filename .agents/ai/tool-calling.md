...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `tool-calling`.
...
flowchart TD
    ...
    Gateway --> Service[ToolCalling Service]
...
## Internal Components
- **`ToolCallingController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`ToolCallingService`**: Core business domain engine executing operational logic.
- **`ToolCallingRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useToolCallingStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
interface ExecuteToolCallingRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteToolCallingResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `ai:tool-calling:update`
- **Listen Event**: `ai:tool-calling:subscribe`
...
```typescript
// Operational Algorithm for Tool Calling Domain Processing
export async function executeToolCallingTask(
  ctx: RequestContext,
  payload: ExecuteToolCallingRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteToolCallingResponseDTO> {
  ...
  const response: ExecuteToolCallingResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `TOOL-CALLING_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `TOOL-CALLING_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `TOOL-CALLING_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
