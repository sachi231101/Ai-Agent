...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `structured-output`.
...
flowchart TD
    ...
    Gateway --> Service[StructuredOutput Service]
...
## Internal Components
- **`StructuredOutputController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`StructuredOutputService`**: Core business domain engine executing operational logic.
- **`StructuredOutputRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useStructuredOutputStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/ai/structured-output/execute
interface ExecuteStructuredOutputRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteStructuredOutputResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `ai:structured-output:update`
- **Listen Event**: `ai:structured-output:subscribe`
...
```typescript
// Operational Algorithm for Structured Output Domain Processing
export async function executeStructuredOutputTask(
  ctx: RequestContext,
  payload: ExecuteStructuredOutputRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteStructuredOutputResponseDTO> {
  ...
  const response: ExecuteStructuredOutputResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `STRUCTURED-OUTPUT_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `STRUCTURED-OUTPUT_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `STRUCTURED-OUTPUT_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
