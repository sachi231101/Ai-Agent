...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `prompt-engineering`.
...
flowchart TD
    ...
    Gateway --> Service[PromptEngineering Service]
...
## Internal Components
- **`PromptEngineeringController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`PromptEngineeringService`**: Core business domain engine executing operational logic.
- **`PromptEngineeringRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`usePromptEngineeringStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/ai/prompt-engineering/execute
interface ExecutePromptEngineeringRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecutePromptEngineeringResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `ai:prompt-engineering:update`
- **Listen Event**: `ai:prompt-engineering:subscribe`
...
```typescript
// Operational Algorithm for Prompt Engineering Domain Processing
export async function executePromptEngineeringTask(
  ctx: RequestContext,
  payload: ExecutePromptEngineeringRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecutePromptEngineeringResponseDTO> {
  ...
  const response: ExecutePromptEngineeringResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `PROMPT-ENGINEERING_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `PROMPT-ENGINEERING_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `PROMPT-ENGINEERING_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
