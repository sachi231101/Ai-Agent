...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `reasoning`.
...
flowchart TD
    ...
    Gateway --> Service[Reasoning Service]
...
## Internal Components
- **`ReasoningController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`ReasoningService`**: Core business domain engine executing operational logic.
- **`ReasoningRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useReasoningStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/ai/reasoning/execute
interface ExecuteReasoningRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteReasoningResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `ai:reasoning:update`
- **Listen Event**: `ai:reasoning:subscribe`
...
```typescript
// Operational Algorithm for Reasoning Domain Processing
export async function executeReasoningTask(
  ctx: RequestContext,
  payload: ExecuteReasoningRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteReasoningResponseDTO> {
  ...
  const response: ExecuteReasoningResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `REASONING_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `REASONING_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `REASONING_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
