...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `evaluation`.
...
flowchart TD
    ...
    Gateway --> Service[Evaluation Service]
...
## Internal Components
- **`EvaluationController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`EvaluationService`**: Core business domain engine executing operational logic.
- **`EvaluationRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useEvaluationStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/ai/evaluation/execute
interface ExecuteEvaluationRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteEvaluationResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `ai:evaluation:update`
- **Listen Event**: `ai:evaluation:subscribe`
...
```typescript
// Operational Algorithm for Evaluation Domain Processing
export async function executeEvaluationTask(
  ctx: RequestContext,
  payload: ExecuteEvaluationRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteEvaluationResponseDTO> {
  ...
  const response: ExecuteEvaluationResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `EVALUATION_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `EVALUATION_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `EVALUATION_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
