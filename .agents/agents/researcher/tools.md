...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `tools`.
...
## Internal Components
- **`ToolsController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`ToolsService`**: Core business domain engine executing operational logic.
- **`ToolsRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useToolsStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
interface ExecuteToolsRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteToolsResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `agents:tools:update`
- **Listen Event**: `agents:tools:subscribe`
...
```typescript
// Operational Algorithm for Tools Domain Processing
export async function executeToolsTask(
  ctx: RequestContext,
  payload: ExecuteToolsRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteToolsResponseDTO> {
  ...
  const response: ExecuteToolsResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `TOOLS_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `TOOLS_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `TOOLS_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
