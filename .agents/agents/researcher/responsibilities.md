...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `responsibilities`.
...
flowchart TD
    ...
    Gateway --> Service[Responsibilities Service]
...
## Internal Components
- **`ResponsibilitiesController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`ResponsibilitiesService`**: Core business domain engine executing operational logic.
- **`ResponsibilitiesRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useResponsibilitiesStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/agents/responsibilities/execute
interface ExecuteResponsibilitiesRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteResponsibilitiesResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `agents:responsibilities:update`
- **Listen Event**: `agents:responsibilities:subscribe`
...
```typescript
// Operational Algorithm for Responsibilities Domain Processing
export async function executeResponsibilitiesTask(
  ctx: RequestContext,
  payload: ExecuteResponsibilitiesRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteResponsibilitiesResponseDTO> {
  ...
  const response: ExecuteResponsibilitiesResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `RESPONSIBILITIES_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `RESPONSIBILITIES_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `RESPONSIBILITIES_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
