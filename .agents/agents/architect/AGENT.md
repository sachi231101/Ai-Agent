...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `AGENT`.
...
## Internal Components
- **`AGENTController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`AGENTService`**: Core business domain engine executing operational logic.
- **`AGENTRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useAGENTStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
interface ExecuteAGENTRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteAGENTResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `agents:AGENT:update`
- **Listen Event**: `agents:AGENT:subscribe`
...
```typescript
// Operational Algorithm for AGENT Domain Processing
export async function executeAGENTTask(
  ctx: RequestContext,
  payload: ExecuteAGENTRequestDTO,
  prisma: PrismaClient,
  redis: Redis
  ...
  const response: ExecuteAGENTResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `AGENT_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `AGENT_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `AGENT_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
