...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `workflow`.
...
flowchart TD
    ...
    Gateway --> Service[Workflow Service]
...
## Internal Components
- **`WorkflowController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`WorkflowService`**: Core business domain engine executing operational logic.
- **`WorkflowRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useWorkflowStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
- **Emit Event**: `agents:workflow:update`
- **Listen Event**: `agents:workflow:subscribe`
...
```typescript
// Operational Algorithm for Workflow Domain Processing
export async function executeWorkflowTask(
  ctx: RequestContext,
  payload: ExecuteWorkflowRequestDTO,
  prisma: PrismaClient,
  redis: Redis
  ...
  const response: ExecuteWorkflowResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `WORKFLOW_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `WORKFLOW_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `WORKFLOW_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
