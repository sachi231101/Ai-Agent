...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `undo-redo`.
...
flowchart TD
    ...
    Gateway --> Service[UndoRedo Service]
...
## Internal Components
- **`UndoRedoController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`UndoRedoService`**: Core business domain engine executing operational logic.
- **`UndoRedoRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useUndoRedoStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/canvas/undo-redo/execute
interface ExecuteUndoRedoRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteUndoRedoResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `canvas:undo-redo:update`
- **Listen Event**: `canvas:undo-redo:subscribe`
...
```typescript
// Operational Algorithm for Undo Redo Domain Processing
export async function executeUndoRedoTask(
  ctx: RequestContext,
  payload: ExecuteUndoRedoRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteUndoRedoResponseDTO> {
  ...
  const response: ExecuteUndoRedoResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `UNDO-REDO_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `UNDO-REDO_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `UNDO-REDO_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
