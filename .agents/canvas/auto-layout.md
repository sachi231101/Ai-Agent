...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `auto-layout`.
...
flowchart TD
    ...
    Gateway --> Service[AutoLayout Service]
...
## Internal Components
- **`AutoLayoutController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`AutoLayoutService`**: Core business domain engine executing operational logic.
- **`AutoLayoutRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useAutoLayoutStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/canvas/auto-layout/execute
interface ExecuteAutoLayoutRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteAutoLayoutResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `canvas:auto-layout:update`
- **Listen Event**: `canvas:auto-layout:subscribe`
...
```typescript
// Operational Algorithm for Auto Layout Domain Processing
export async function executeAutoLayoutTask(
  ctx: RequestContext,
  payload: ExecuteAutoLayoutRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteAutoLayoutResponseDTO> {
  ...
  const response: ExecuteAutoLayoutResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `AUTO-LAYOUT_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `AUTO-LAYOUT_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `AUTO-LAYOUT_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
