...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `canvas-state`.
...
flowchart TD
    ...
    Gateway --> Service[CanvasState Service]
...
## Internal Components
- **`CanvasStateController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`CanvasStateService`**: Core business domain engine executing operational logic.
- **`CanvasStateRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useCanvasStateStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/canvas/canvas-state/execute
interface ExecuteCanvasStateRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteCanvasStateResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `canvas:canvas-state:update`
- **Listen Event**: `canvas:canvas-state:subscribe`
...
```typescript
// Operational Algorithm for Canvas State Domain Processing
export async function executeCanvasStateTask(
  ctx: RequestContext,
  payload: ExecuteCanvasStateRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteCanvasStateResponseDTO> {
  ...
  const response: ExecuteCanvasStateResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `CANVAS-STATE_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `CANVAS-STATE_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `CANVAS-STATE_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
