...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `node-selection`.
...
flowchart TD
    ...
    Gateway --> Service[NodeSelection Service]
...
## Internal Components
- **`NodeSelectionController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`NodeSelectionService`**: Core business domain engine executing operational logic.
- **`NodeSelectionRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useNodeSelectionStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/canvas/node-selection/execute
interface ExecuteNodeSelectionRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteNodeSelectionResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `canvas:node-selection:update`
- **Listen Event**: `canvas:node-selection:subscribe`
...
```typescript
// Operational Algorithm for Node Selection Domain Processing
export async function executeNodeSelectionTask(
  ctx: RequestContext,
  payload: ExecuteNodeSelectionRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteNodeSelectionResponseDTO> {
  ...
  const response: ExecuteNodeSelectionResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `NODE-SELECTION_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `NODE-SELECTION_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `NODE-SELECTION_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
