...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `shortcuts`.
...
flowchart TD
    ...
    Gateway --> Service[Shortcuts Service]
...
## Internal Components
- **`ShortcutsController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`ShortcutsService`**: Core business domain engine executing operational logic.
- **`ShortcutsRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useShortcutsStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/canvas/shortcuts/execute
interface ExecuteShortcutsRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteShortcutsResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `canvas:shortcuts:update`
- **Listen Event**: `canvas:shortcuts:subscribe`
...
```typescript
// Operational Algorithm for Shortcuts Domain Processing
export async function executeShortcutsTask(
  ctx: RequestContext,
  payload: ExecuteShortcutsRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteShortcutsResponseDTO> {
  ...
  const response: ExecuteShortcutsResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `SHORTCUTS_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `SHORTCUTS_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `SHORTCUTS_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
