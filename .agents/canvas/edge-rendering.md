...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `edge-rendering`.
...
flowchart TD
    ...
    Gateway --> Service[EdgeRendering Service]
...
## Internal Components
- **`EdgeRenderingController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`EdgeRenderingService`**: Core business domain engine executing operational logic.
- **`EdgeRenderingRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useEdgeRenderingStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/canvas/edge-rendering/execute
interface ExecuteEdgeRenderingRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteEdgeRenderingResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `canvas:edge-rendering:update`
- **Listen Event**: `canvas:edge-rendering:subscribe`
...
```typescript
// Operational Algorithm for Edge Rendering Domain Processing
export async function executeEdgeRenderingTask(
  ctx: RequestContext,
  payload: ExecuteEdgeRenderingRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteEdgeRenderingResponseDTO> {
  ...
  const response: ExecuteEdgeRenderingResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `EDGE-RENDERING_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `EDGE-RENDERING_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `EDGE-RENDERING_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
