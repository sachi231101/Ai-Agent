...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `drag-drop`.
...
flowchart TD
    ...
    Gateway --> Service[DragDrop Service]
...
## Internal Components
- **`DragDropController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`DragDropService`**: Core business domain engine executing operational logic.
- **`DragDropRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useDragDropStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/canvas/drag-drop/execute
interface ExecuteDragDropRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteDragDropResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `canvas:drag-drop:update`
- **Listen Event**: `canvas:drag-drop:subscribe`
...
```typescript
// Operational Algorithm for Drag Drop Domain Processing
export async function executeDragDropTask(
  ctx: RequestContext,
  payload: ExecuteDragDropRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteDragDropResponseDTO> {
  ...
  const response: ExecuteDragDropResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `DRAG-DROP_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `DRAG-DROP_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `DRAG-DROP_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
