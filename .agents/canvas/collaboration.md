...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `collaboration`.
...
flowchart TD
    ...
    Gateway --> Service[Collaboration Service]
...
## Internal Components
- **`CollaborationController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`CollaborationService`**: Core business domain engine executing operational logic.
- **`CollaborationRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useCollaborationStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/canvas/collaboration/execute
interface ExecuteCollaborationRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteCollaborationResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `canvas:collaboration:update`
- **Listen Event**: `canvas:collaboration:subscribe`
...
```typescript
// Operational Algorithm for Collaboration Domain Processing
export async function executeCollaborationTask(
  ctx: RequestContext,
  payload: ExecuteCollaborationRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteCollaborationResponseDTO> {
  ...
  const response: ExecuteCollaborationResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `COLLABORATION_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `COLLABORATION_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `COLLABORATION_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
