...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `node-rendering`.
...
flowchart TD
    ...
    Gateway --> Service[NodeRendering Service]
...
## Internal Components
- **`NodeRenderingController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`NodeRenderingService`**: Core business domain engine executing operational logic.
- **`NodeRenderingRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useNodeRenderingStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/canvas/node-rendering/execute
interface ExecuteNodeRenderingRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteNodeRenderingResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `canvas:node-rendering:update`
- **Listen Event**: `canvas:node-rendering:subscribe`
...
```typescript
// Operational Algorithm for Node Rendering Domain Processing
export async function executeNodeRenderingTask(
  ctx: RequestContext,
  payload: ExecuteNodeRenderingRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteNodeRenderingResponseDTO> {
  ...
  const response: ExecuteNodeRenderingResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `NODE-RENDERING_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `NODE-RENDERING_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `NODE-RENDERING_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
