...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `slack`.
...
## Internal Components
- **`SlackController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`SlackService`**: Core business domain engine executing operational logic.
- **`SlackRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useSlackStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/integrations/slack/execute
interface ExecuteSlackRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteSlackResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `integrations:slack:update`
- **Listen Event**: `integrations:slack:subscribe`
...
```typescript
// Operational Algorithm for Slack Domain Processing
export async function executeSlackTask(
  ctx: RequestContext,
  payload: ExecuteSlackRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteSlackResponseDTO> {
  ...
  const response: ExecuteSlackResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `SLACK_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `SLACK_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `SLACK_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
