...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `custom-integrations`.
...
flowchart TD
    ...
    Gateway --> Service[CustomIntegrations Service]
...
## Internal Components
- **`CustomIntegrationsController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`CustomIntegrationsService`**: Core business domain engine executing operational logic.
- **`CustomIntegrationsRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useCustomIntegrationsStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/integrations/custom-integrations/execute
interface ExecuteCustomIntegrationsRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteCustomIntegrationsResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `integrations:custom-integrations:update`
- **Listen Event**: `integrations:custom-integrations:subscribe`
...
```typescript
// Operational Algorithm for Custom Integrations Domain Processing
export async function executeCustomIntegrationsTask(
  ctx: RequestContext,
  payload: ExecuteCustomIntegrationsRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteCustomIntegrationsResponseDTO> {
  ...
  const response: ExecuteCustomIntegrationsResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `CUSTOM-INTEGRATIONS_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `CUSTOM-INTEGRATIONS_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `CUSTOM-INTEGRATIONS_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
