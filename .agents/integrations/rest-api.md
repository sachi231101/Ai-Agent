...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `rest-api`.
...
flowchart TD
    ...
    Gateway --> Service[RestApi Service]
...
## Internal Components
- **`RestApiController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`RestApiService`**: Core business domain engine executing operational logic.
- **`RestApiRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useRestApiStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/integrations/rest-api/execute
interface ExecuteRestApiRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteRestApiResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `integrations:rest-api:update`
- **Listen Event**: `integrations:rest-api:subscribe`
...
```typescript
// Operational Algorithm for Rest Api Domain Processing
export async function executeRestApiTask(
  ctx: RequestContext,
  payload: ExecuteRestApiRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteRestApiResponseDTO> {
  ...
  const response: ExecuteRestApiResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `REST-API_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `REST-API_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `REST-API_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
