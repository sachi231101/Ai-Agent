...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `graphql`.
...
flowchart TD
    ...
    Gateway --> Service[Graphql Service]
...
## Internal Components
- **`GraphqlController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`GraphqlService`**: Core business domain engine executing operational logic.
- **`GraphqlRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useGraphqlStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/integrations/graphql/execute
interface ExecuteGraphqlRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteGraphqlResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `integrations:graphql:update`
- **Listen Event**: `integrations:graphql:subscribe`
...
```typescript
// Operational Algorithm for Graphql Domain Processing
export async function executeGraphqlTask(
  ctx: RequestContext,
  payload: ExecuteGraphqlRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteGraphqlResponseDTO> {
  ...
  const response: ExecuteGraphqlResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `GRAPHQL_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `GRAPHQL_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `GRAPHQL_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
