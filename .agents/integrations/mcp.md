...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `mcp`.
...
## Internal Components
- **`McpController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`McpService`**: Core business domain engine executing operational logic.
- **`McpRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useMcpStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
interface ExecuteMcpRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteMcpResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `integrations:mcp:update`
- **Listen Event**: `integrations:mcp:subscribe`
...
```typescript
// Operational Algorithm for Mcp Domain Processing
export async function executeMcpTask(
  ctx: RequestContext,
  payload: ExecuteMcpRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteMcpResponseDTO> {
  ...
  const response: ExecuteMcpResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `MCP_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `MCP_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `MCP_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
