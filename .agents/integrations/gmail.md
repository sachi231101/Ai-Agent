...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `gmail`.
...
## Internal Components
- **`GmailController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`GmailService`**: Core business domain engine executing operational logic.
- **`GmailRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useGmailStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/integrations/gmail/execute
interface ExecuteGmailRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteGmailResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `integrations:gmail:update`
- **Listen Event**: `integrations:gmail:subscribe`
...
```typescript
// Operational Algorithm for Gmail Domain Processing
export async function executeGmailTask(
  ctx: RequestContext,
  payload: ExecuteGmailRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteGmailResponseDTO> {
  ...
  const response: ExecuteGmailResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `GMAIL_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `GMAIL_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `GMAIL_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
