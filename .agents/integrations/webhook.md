...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `webhook`.
...
flowchart TD
    ...
    Gateway --> Service[Webhook Service]
...
## Internal Components
- **`WebhookController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`WebhookService`**: Core business domain engine executing operational logic.
- **`WebhookRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useWebhookStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/integrations/webhook/execute
interface ExecuteWebhookRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteWebhookResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `integrations:webhook:update`
- **Listen Event**: `integrations:webhook:subscribe`
...
```typescript
// Operational Algorithm for Webhook Domain Processing
export async function executeWebhookTask(
  ctx: RequestContext,
  payload: ExecuteWebhookRequestDTO,
  prisma: PrismaClient,
  redis: Redis
  ...
  const response: ExecuteWebhookResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `WEBHOOK_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `WEBHOOK_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `WEBHOOK_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
