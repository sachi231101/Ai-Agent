...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `discord`.
...
flowchart TD
    ...
    Gateway --> Service[Discord Service]
...
## Internal Components
- **`DiscordController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`DiscordService`**: Core business domain engine executing operational logic.
- **`DiscordRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useDiscordStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/integrations/discord/execute
interface ExecuteDiscordRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteDiscordResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `integrations:discord:update`
- **Listen Event**: `integrations:discord:subscribe`
...
```typescript
// Operational Algorithm for Discord Domain Processing
export async function executeDiscordTask(
  ctx: RequestContext,
  payload: ExecuteDiscordRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteDiscordResponseDTO> {
  ...
  const response: ExecuteDiscordResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `DISCORD_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `DISCORD_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `DISCORD_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
