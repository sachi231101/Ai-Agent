...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `oauth`.
...
## Internal Components
- **`OauthController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`OauthService`**: Core business domain engine executing operational logic.
- **`OauthRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useOauthStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/integrations/oauth/execute
interface ExecuteOauthRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteOauthResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `integrations:oauth:update`
- **Listen Event**: `integrations:oauth:subscribe`
...
```typescript
// Operational Algorithm for Oauth Domain Processing
export async function executeOauthTask(
  ctx: RequestContext,
  payload: ExecuteOauthRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteOauthResponseDTO> {
  ...
  const response: ExecuteOauthResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `OAUTH_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `OAUTH_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `OAUTH_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
