...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `notion`.
...
flowchart TD
    ...
    Gateway --> Service[Notion Service]
...
## Internal Components
- **`NotionController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`NotionService`**: Core business domain engine executing operational logic.
- **`NotionRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useNotionStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
interface ExecuteNotionRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteNotionResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `integrations:notion:update`
- **Listen Event**: `integrations:notion:subscribe`
...
```typescript
// Operational Algorithm for Notion Domain Processing
export async function executeNotionTask(
  ctx: RequestContext,
  payload: ExecuteNotionRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteNotionResponseDTO> {
  ...
  const response: ExecuteNotionResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `NOTION_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `NOTION_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `NOTION_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
