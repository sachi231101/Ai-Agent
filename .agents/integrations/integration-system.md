...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `integration-system`.
...
flowchart TD
    ...
    Gateway --> Service[IntegrationSystem Service]
...
## Internal Components
- **`IntegrationSystemController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`IntegrationSystemService`**: Core business domain engine executing operational logic.
- **`IntegrationSystemRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useIntegrationSystemStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/integrations/integration-system/execute
interface ExecuteIntegrationSystemRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteIntegrationSystemResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `integrations:integration-system:update`
- **Listen Event**: `integrations:integration-system:subscribe`
...
```typescript
// Operational Algorithm for Integration System Domain Processing
export async function executeIntegrationSystemTask(
  ctx: RequestContext,
  payload: ExecuteIntegrationSystemRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteIntegrationSystemResponseDTO> {
  ...
  const response: ExecuteIntegrationSystemResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `INTEGRATION-SYSTEM_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `INTEGRATION-SYSTEM_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `INTEGRATION-SYSTEM_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
