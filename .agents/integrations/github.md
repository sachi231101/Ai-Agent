...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `github`.
...
flowchart TD
    ...
    Gateway --> Service[Github Service]
...
## Internal Components
- **`GithubController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`GithubService`**: Core business domain engine executing operational logic.
- **`GithubRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useGithubStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/integrations/github/execute
interface ExecuteGithubRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteGithubResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `integrations:github:update`
- **Listen Event**: `integrations:github:subscribe`
...
```typescript
// Operational Algorithm for Github Domain Processing
export async function executeGithubTask(
  ctx: RequestContext,
  payload: ExecuteGithubRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteGithubResponseDTO> {
  ...
  const response: ExecuteGithubResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `GITHUB_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `GITHUB_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `GITHUB_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
