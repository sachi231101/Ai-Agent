...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `reviews`.
...
flowchart TD
    ...
    Gateway --> Service[Reviews Service]
...
## Internal Components
- **`ReviewsController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`ReviewsService`**: Core business domain engine executing operational logic.
- **`ReviewsRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useReviewsStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/marketplace/reviews/execute
interface ExecuteReviewsRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteReviewsResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `marketplace:reviews:update`
- **Listen Event**: `marketplace:reviews:subscribe`
...
```typescript
// Operational Algorithm for Reviews Domain Processing
export async function executeReviewsTask(
  ctx: RequestContext,
  payload: ExecuteReviewsRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteReviewsResponseDTO> {
  ...
  const response: ExecuteReviewsResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `REVIEWS_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `REVIEWS_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `REVIEWS_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
