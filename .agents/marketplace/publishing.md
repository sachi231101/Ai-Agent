...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `publishing`.
...
flowchart TD
    ...
    Gateway --> Service[Publishing Service]
...
## Internal Components
- **`PublishingController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`PublishingService`**: Core business domain engine executing operational logic.
- **`PublishingRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`usePublishingStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/marketplace/publishing/execute
interface ExecutePublishingRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecutePublishingResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `marketplace:publishing:update`
- **Listen Event**: `marketplace:publishing:subscribe`
...
```typescript
// Operational Algorithm for Publishing Domain Processing
export async function executePublishingTask(
  ctx: RequestContext,
  payload: ExecutePublishingRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecutePublishingResponseDTO> {
  ...
  const response: ExecutePublishingResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `PUBLISHING_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `PUBLISHING_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `PUBLISHING_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
