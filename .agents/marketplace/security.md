...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `security`.
...
flowchart TD
    ...
    Gateway --> Service[Security Service]
...
## Internal Components
- **`SecurityController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`SecurityService`**: Core business domain engine executing operational logic.
- **`SecurityRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useSecurityStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/marketplace/security/execute
interface ExecuteSecurityRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteSecurityResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `marketplace:security:update`
- **Listen Event**: `marketplace:security:subscribe`
...
```typescript
// Operational Algorithm for Security Domain Processing
export async function executeSecurityTask(
  ctx: RequestContext,
  payload: ExecuteSecurityRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteSecurityResponseDTO> {
  ...
  const response: ExecuteSecurityResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `SECURITY_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `SECURITY_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `SECURITY_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
