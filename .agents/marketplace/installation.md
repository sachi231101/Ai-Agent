...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `installation`.
...
flowchart TD
    ...
    Gateway --> Service[Installation Service]
...
## Internal Components
- **`InstallationController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`InstallationService`**: Core business domain engine executing operational logic.
- **`InstallationRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useInstallationStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/marketplace/installation/execute
interface ExecuteInstallationRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteInstallationResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `marketplace:installation:update`
- **Listen Event**: `marketplace:installation:subscribe`
...
```typescript
// Operational Algorithm for Installation Domain Processing
export async function executeInstallationTask(
  ctx: RequestContext,
  payload: ExecuteInstallationRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteInstallationResponseDTO> {
  ...
  const response: ExecuteInstallationResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `INSTALLATION_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `INSTALLATION_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `INSTALLATION_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
