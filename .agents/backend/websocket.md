...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `websocket`.
...
## Internal Components
- **`WebsocketController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`WebsocketService`**: Core business domain engine executing operational logic.
- **`WebsocketRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useWebsocketStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/backend/websocket/execute
interface ExecuteWebsocketRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteWebsocketResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `backend:websocket:update`
- **Listen Event**: `backend:websocket:subscribe`
...
```typescript
// Operational Algorithm for Websocket Domain Processing
export async function executeWebsocketTask(
  ctx: RequestContext,
  payload: ExecuteWebsocketRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteWebsocketResponseDTO> {
  ...
  const response: ExecuteWebsocketResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `WEBSOCKET_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `WEBSOCKET_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `WEBSOCKET_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
