...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `events`.
...
flowchart TD
    ...
    Gateway --> Service[Events Service]
...
## Internal Components
- **`EventsController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`EventsService`**: Core business domain engine executing operational logic.
- **`EventsRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useEventsStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/backend/events/execute
interface ExecuteEventsRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteEventsResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `backend:events:update`
- **Listen Event**: `backend:events:subscribe`
...
```typescript
// Operational Algorithm for Events Domain Processing
export async function executeEventsTask(
  ctx: RequestContext,
  payload: ExecuteEventsRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteEventsResponseDTO> {
  ...
  const response: ExecuteEventsResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `EVENTS_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `EVENTS_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `EVENTS_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
