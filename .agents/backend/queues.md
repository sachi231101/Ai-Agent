...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `queues`.
...
flowchart TD
    ...
    Gateway --> Service[Queues Service]
...
## Internal Components
- **`QueuesController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`QueuesService`**: Core business domain engine executing operational logic.
- **`QueuesRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useQueuesStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/backend/queues/execute
interface ExecuteQueuesRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteQueuesResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `backend:queues:update`
- **Listen Event**: `backend:queues:subscribe`
...
```typescript
// Operational Algorithm for Queues Domain Processing
export async function executeQueuesTask(
  ctx: RequestContext,
  payload: ExecuteQueuesRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteQueuesResponseDTO> {
  ...
  const response: ExecuteQueuesResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `QUEUES_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `QUEUES_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `QUEUES_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
