...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `nodejs`.
...
flowchart TD
    ...
    Gateway --> Service[Nodejs Service]
...
## Internal Components
- **`NodejsController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`NodejsService`**: Core business domain engine executing operational logic.
- **`NodejsRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useNodejsStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/backend/nodejs/execute
interface ExecuteNodejsRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteNodejsResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `backend:nodejs:update`
- **Listen Event**: `backend:nodejs:subscribe`
...
```typescript
// Operational Algorithm for Nodejs Domain Processing
export async function executeNodejsTask(
  ctx: RequestContext,
  payload: ExecuteNodejsRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteNodejsResponseDTO> {
  ...
  const response: ExecuteNodejsResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `NODEJS_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `NODEJS_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `NODEJS_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
