...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `prisma`.
...
flowchart TD
    ...
    Gateway --> Service[Prisma Service]
...
## Internal Components
- **`PrismaController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`PrismaService`**: Core business domain engine executing operational logic.
- **`PrismaRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`usePrismaStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/backend/prisma/execute
interface ExecutePrismaRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecutePrismaResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `backend:prisma:update`
- **Listen Event**: `backend:prisma:subscribe`
...
```typescript
// Operational Algorithm for Prisma Domain Processing
export async function executePrismaTask(
  ctx: RequestContext,
  payload: ExecutePrismaRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecutePrismaResponseDTO> {
  ...
  const response: ExecutePrismaResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `PRISMA_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `PRISMA_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `PRISMA_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
