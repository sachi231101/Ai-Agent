...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `scheduler`.
...
flowchart TD
    ...
    Gateway --> Service[Scheduler Service]
...
## Internal Components
- **`SchedulerController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`SchedulerService`**: Core business domain engine executing operational logic.
- **`SchedulerRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useSchedulerStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/backend/scheduler/execute
interface ExecuteSchedulerRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteSchedulerResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `backend:scheduler:update`
- **Listen Event**: `backend:scheduler:subscribe`
...
```typescript
// Operational Algorithm for Scheduler Domain Processing
export async function executeSchedulerTask(
  ctx: RequestContext,
  payload: ExecuteSchedulerRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteSchedulerResponseDTO> {
  ...
  const response: ExecuteSchedulerResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `SCHEDULER_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `SCHEDULER_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `SCHEDULER_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
