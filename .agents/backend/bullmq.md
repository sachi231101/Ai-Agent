...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `bullmq`.
...
flowchart TD
    ...
    Gateway --> Service[Bullmq Service]
...
## Internal Components
- **`BullmqController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`BullmqService`**: Core business domain engine executing operational logic.
- **`BullmqRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useBullmqStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/backend/bullmq/execute
interface ExecuteBullmqRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteBullmqResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `backend:bullmq:update`
- **Listen Event**: `backend:bullmq:subscribe`
...
```typescript
// Operational Algorithm for Bullmq Domain Processing
export async function executeBullmqTask(
  ctx: RequestContext,
  payload: ExecuteBullmqRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteBullmqResponseDTO> {
  ...
  const response: ExecuteBullmqResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `BULLMQ_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `BULLMQ_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `BULLMQ_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
