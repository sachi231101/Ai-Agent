...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `postgres`.
...
flowchart TD
    ...
    Gateway --> Service[Postgres Service]
...
## Internal Components
- **`PostgresController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`PostgresService`**: Core business domain engine executing operational logic.
- **`PostgresRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`usePostgresStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/backend/postgres/execute
interface ExecutePostgresRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecutePostgresResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `backend:postgres:update`
- **Listen Event**: `backend:postgres:subscribe`
...
```typescript
// Operational Algorithm for Postgres Domain Processing
export async function executePostgresTask(
  ctx: RequestContext,
  payload: ExecutePostgresRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecutePostgresResponseDTO> {
  ...
  const response: ExecutePostgresResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `POSTGRES_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `POSTGRES_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `POSTGRES_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
