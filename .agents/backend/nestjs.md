...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `nestjs`.
...
flowchart TD
    ...
    Gateway --> Service[Nestjs Service]
...
## Internal Components
- **`NestjsController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`NestjsService`**: Core business domain engine executing operational logic.
- **`NestjsRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useNestjsStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/backend/nestjs/execute
interface ExecuteNestjsRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteNestjsResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `backend:nestjs:update`
- **Listen Event**: `backend:nestjs:subscribe`
...
```typescript
// Operational Algorithm for Nestjs Domain Processing
export async function executeNestjsTask(
  ctx: RequestContext,
  payload: ExecuteNestjsRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteNestjsResponseDTO> {
  ...
  const response: ExecuteNestjsResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `NESTJS_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `NESTJS_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `NESTJS_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
