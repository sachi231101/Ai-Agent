...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `redis`.
...
## Internal Components
- **`RedisController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`RedisService`**: Core business domain engine executing operational logic.
- **`RedisRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useRedisStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/backend/redis/execute
interface ExecuteRedisRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteRedisResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `backend:redis:update`
- **Listen Event**: `backend:redis:subscribe`
...
```typescript
// Operational Algorithm for Redis Domain Processing
export async function executeRedisTask(
  ctx: RequestContext,
  payload: ExecuteRedisRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteRedisResponseDTO> {
  ...
  const response: ExecuteRedisResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `REDIS_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `REDIS_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `REDIS_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
