...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `services`.
...
flowchart TD
    ...
    Gateway --> Service[Services Service]
...
## Internal Components
- **`ServicesController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`ServicesService`**: Core business domain engine executing operational logic.
- **`ServicesRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useServicesStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/backend/services/execute
interface ExecuteServicesRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteServicesResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `backend:services:update`
- **Listen Event**: `backend:services:subscribe`
...
```typescript
// Operational Algorithm for Services Domain Processing
export async function executeServicesTask(
  ctx: RequestContext,
  payload: ExecuteServicesRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteServicesResponseDTO> {
  ...
  const response: ExecuteServicesResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `SERVICES_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `SERVICES_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `SERVICES_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
