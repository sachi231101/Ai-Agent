...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `api-design`.
...
flowchart TD
    ...
    Gateway --> Service[ApiDesign Service]
...
## Internal Components
- **`ApiDesignController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`ApiDesignService`**: Core business domain engine executing operational logic.
- **`ApiDesignRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useApiDesignStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/backend/api-design/execute
interface ExecuteApiDesignRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteApiDesignResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `backend:api-design:update`
- **Listen Event**: `backend:api-design:subscribe`
...
```typescript
// Operational Algorithm for Api Design Domain Processing
export async function executeApiDesignTask(
  ctx: RequestContext,
  payload: ExecuteApiDesignRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteApiDesignResponseDTO> {
  ...
  const response: ExecuteApiDesignResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `API-DESIGN_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `API-DESIGN_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `API-DESIGN_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
