...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `zustand`.
...
flowchart TD
    ...
    Gateway --> Service[Zustand Service]
...
## Internal Components
- **`ZustandController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`ZustandService`**: Core business domain engine executing operational logic.
- **`ZustandRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useZustandStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/frontend/zustand/execute
interface ExecuteZustandRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteZustandResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `frontend:zustand:update`
- **Listen Event**: `frontend:zustand:subscribe`
...
```typescript
// Operational Algorithm for Zustand Domain Processing
export async function executeZustandTask(
  ctx: RequestContext,
  payload: ExecuteZustandRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteZustandResponseDTO> {
  ...
  const response: ExecuteZustandResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `ZUSTAND_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `ZUSTAND_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `ZUSTAND_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
