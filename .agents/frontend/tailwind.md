...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `tailwind`.
...
flowchart TD
    ...
    Gateway --> Service[Tailwind Service]
...
## Internal Components
- **`TailwindController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`TailwindService`**: Core business domain engine executing operational logic.
- **`TailwindRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useTailwindStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/frontend/tailwind/execute
interface ExecuteTailwindRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteTailwindResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `frontend:tailwind:update`
- **Listen Event**: `frontend:tailwind:subscribe`
...
```typescript
// Operational Algorithm for Tailwind Domain Processing
export async function executeTailwindTask(
  ctx: RequestContext,
  payload: ExecuteTailwindRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteTailwindResponseDTO> {
  ...
  const response: ExecuteTailwindResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `TAILWIND_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `TAILWIND_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `TAILWIND_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
