...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `vite`.
...
## Internal Components
- **`ViteController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`ViteService`**: Core business domain engine executing operational logic.
- **`ViteRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useViteStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/frontend/vite/execute
interface ExecuteViteRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteViteResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `frontend:vite:update`
- **Listen Event**: `frontend:vite:subscribe`
...
```typescript
// Operational Algorithm for Vite Domain Processing
export async function executeViteTask(
  ctx: RequestContext,
  payload: ExecuteViteRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteViteResponseDTO> {
  ...
  const response: ExecuteViteResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `VITE_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `VITE_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `VITE_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
