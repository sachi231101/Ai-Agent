...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `routing`.
...
flowchart TD
    ...
    Gateway --> Service[Routing Service]
...
## Internal Components
- **`RoutingController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`RoutingService`**: Core business domain engine executing operational logic.
- **`RoutingRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useRoutingStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/frontend/routing/execute
interface ExecuteRoutingRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteRoutingResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `frontend:routing:update`
- **Listen Event**: `frontend:routing:subscribe`
...
```typescript
// Operational Algorithm for Routing Domain Processing
export async function executeRoutingTask(
  ctx: RequestContext,
  payload: ExecuteRoutingRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteRoutingResponseDTO> {
  ...
  const response: ExecuteRoutingResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `ROUTING_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `ROUTING_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `ROUTING_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
