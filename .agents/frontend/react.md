...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `react`.
...
## Internal Components
- **`ReactController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`ReactService`**: Core business domain engine executing operational logic.
- **`ReactRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useReactStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/frontend/react/execute
interface ExecuteReactRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteReactResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `frontend:react:update`
- **Listen Event**: `frontend:react:subscribe`
...
```typescript
// Operational Algorithm for React Domain Processing
export async function executeReactTask(
  ctx: RequestContext,
  payload: ExecuteReactRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteReactResponseDTO> {
  ...
  const response: ExecuteReactResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `REACT_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `REACT_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `REACT_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
