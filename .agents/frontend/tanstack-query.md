...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `tanstack-query`.
...
flowchart TD
    ...
    Gateway --> Service[TanstackQuery Service]
...
## Internal Components
- **`TanstackQueryController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`TanstackQueryService`**: Core business domain engine executing operational logic.
- **`TanstackQueryRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useTanstackQueryStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/frontend/tanstack-query/execute
interface ExecuteTanstackQueryRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteTanstackQueryResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `frontend:tanstack-query:update`
- **Listen Event**: `frontend:tanstack-query:subscribe`
...
```typescript
// Operational Algorithm for Tanstack Query Domain Processing
export async function executeTanstackQueryTask(
  ctx: RequestContext,
  payload: ExecuteTanstackQueryRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteTanstackQueryResponseDTO> {
  ...
  const response: ExecuteTanstackQueryResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `TANSTACK-QUERY_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `TANSTACK-QUERY_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `TANSTACK-QUERY_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
