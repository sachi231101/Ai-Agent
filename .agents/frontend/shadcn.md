...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `shadcn`.
...
flowchart TD
    ...
    Gateway --> Service[Shadcn Service]
...
## Internal Components
- **`ShadcnController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`ShadcnService`**: Core business domain engine executing operational logic.
- **`ShadcnRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useShadcnStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/frontend/shadcn/execute
interface ExecuteShadcnRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteShadcnResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `frontend:shadcn:update`
- **Listen Event**: `frontend:shadcn:subscribe`
...
```typescript
// Operational Algorithm for Shadcn Domain Processing
export async function executeShadcnTask(
  ctx: RequestContext,
  payload: ExecuteShadcnRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteShadcnResponseDTO> {
  ...
  const response: ExecuteShadcnResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `SHADCN_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `SHADCN_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `SHADCN_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
