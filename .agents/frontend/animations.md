...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `animations`.
...
flowchart TD
    ...
    Gateway --> Service[Animations Service]
...
## Internal Components
- **`AnimationsController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`AnimationsService`**: Core business domain engine executing operational logic.
- **`AnimationsRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useAnimationsStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/frontend/animations/execute
interface ExecuteAnimationsRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteAnimationsResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `frontend:animations:update`
- **Listen Event**: `frontend:animations:subscribe`
...
```typescript
// Operational Algorithm for Animations Domain Processing
export async function executeAnimationsTask(
  ctx: RequestContext,
  payload: ExecuteAnimationsRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteAnimationsResponseDTO> {
  ...
  const response: ExecuteAnimationsResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `ANIMATIONS_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `ANIMATIONS_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `ANIMATIONS_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
