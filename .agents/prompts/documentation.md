...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `documentation`.
...
flowchart TD
    ...
    Gateway --> Service[Documentation Service]
...
## Internal Components
- **`DocumentationController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`DocumentationService`**: Core business domain engine executing operational logic.
- **`DocumentationRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useDocumentationStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/prompts/documentation/execute
interface ExecuteDocumentationRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteDocumentationResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `prompts:documentation:update`
- **Listen Event**: `prompts:documentation:subscribe`
...
```typescript
// Operational Algorithm for Documentation Domain Processing
export async function executeDocumentationTask(
  ctx: RequestContext,
  payload: ExecuteDocumentationRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteDocumentationResponseDTO> {
  ...
  const response: ExecuteDocumentationResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `DOCUMENTATION_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `DOCUMENTATION_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `DOCUMENTATION_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
