...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `backend`.
...
## Internal Components
- **`BackendController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`BackendService`**: Core business domain engine executing operational logic.
- **`BackendRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useBackendStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/prompts/backend/execute
interface ExecuteBackendRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteBackendResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `prompts:backend:update`
- **Listen Event**: `prompts:backend:subscribe`
...
```typescript
// Operational Algorithm for Backend Domain Processing
export async function executeBackendTask(
  ctx: RequestContext,
  payload: ExecuteBackendRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteBackendResponseDTO> {
  ...
  const response: ExecuteBackendResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `BACKEND_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `BACKEND_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `BACKEND_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
