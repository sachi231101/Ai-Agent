...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `debugger`.
...
flowchart TD
    ...
    Gateway --> Service[Debugger Service]
...
## Internal Components
- **`DebuggerController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`DebuggerService`**: Core business domain engine executing operational logic.
- **`DebuggerRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useDebuggerStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/prompts/debugger/execute
interface ExecuteDebuggerRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteDebuggerResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `prompts:debugger:update`
- **Listen Event**: `prompts:debugger:subscribe`
...
```typescript
// Operational Algorithm for Debugger Domain Processing
export async function executeDebuggerTask(
  ctx: RequestContext,
  payload: ExecuteDebuggerRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteDebuggerResponseDTO> {
  ...
  const response: ExecuteDebuggerResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `DEBUGGER_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `DEBUGGER_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `DEBUGGER_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
