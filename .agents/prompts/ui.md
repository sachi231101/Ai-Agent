...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `ui`.
...
## Internal Components
- **`UiController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`UiService`**: Core business domain engine executing operational logic.
- **`UiRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useUiStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/prompts/ui/execute
interface ExecuteUiRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteUiResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `prompts:ui:update`
- **Listen Event**: `prompts:ui:subscribe`
...
```typescript
// Operational Algorithm for Ui Domain Processing
export async function executeUiTask(
  ctx: RequestContext,
  payload: ExecuteUiRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteUiResponseDTO> {
  ...
  const response: ExecuteUiResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `UI_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `UI_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `UI_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
