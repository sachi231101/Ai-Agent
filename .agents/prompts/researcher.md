...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `researcher`.
...
flowchart TD
    ...
    Gateway --> Service[Researcher Service]
...
## Internal Components
- **`ResearcherController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`ResearcherService`**: Core business domain engine executing operational logic.
- **`ResearcherRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useResearcherStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/prompts/researcher/execute
interface ExecuteResearcherRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteResearcherResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `prompts:researcher:update`
- **Listen Event**: `prompts:researcher:subscribe`
...
```typescript
// Operational Algorithm for Researcher Domain Processing
export async function executeResearcherTask(
  ctx: RequestContext,
  payload: ExecuteResearcherRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteResearcherResponseDTO> {
  ...
  const response: ExecuteResearcherResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `RESEARCHER_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `RESEARCHER_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `RESEARCHER_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
