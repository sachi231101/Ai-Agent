...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `architect`.
...
flowchart TD
    ...
    Gateway --> Service[Architect Service]
...
## Internal Components
- **`ArchitectController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`ArchitectService`**: Core business domain engine executing operational logic.
- **`ArchitectRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useArchitectStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/prompts/architect/execute
interface ExecuteArchitectRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteArchitectResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `prompts:architect:update`
- **Listen Event**: `prompts:architect:subscribe`
...
```typescript
// Operational Algorithm for Architect Domain Processing
export async function executeArchitectTask(
  ctx: RequestContext,
  payload: ExecuteArchitectRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteArchitectResponseDTO> {
  ...
  const response: ExecuteArchitectResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `ARCHITECT_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `ARCHITECT_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `ARCHITECT_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
