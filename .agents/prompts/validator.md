...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `validator`.
...
flowchart TD
    ...
    Gateway --> Service[Validator Service]
...
## Internal Components
- **`ValidatorController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`ValidatorService`**: Core business domain engine executing operational logic.
- **`ValidatorRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useValidatorStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/prompts/validator/execute
interface ExecuteValidatorRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteValidatorResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `prompts:validator:update`
- **Listen Event**: `prompts:validator:subscribe`
...
```typescript
// Operational Algorithm for Validator Domain Processing
export async function executeValidatorTask(
  ctx: RequestContext,
  payload: ExecuteValidatorRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteValidatorResponseDTO> {
  ...
  const response: ExecuteValidatorResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `VALIDATOR_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `VALIDATOR_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `VALIDATOR_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
