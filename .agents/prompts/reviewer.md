...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `reviewer`.
...
flowchart TD
    ...
    Gateway --> Service[Reviewer Service]
...
## Internal Components
- **`ReviewerController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`ReviewerService`**: Core business domain engine executing operational logic.
- **`ReviewerRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useReviewerStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/prompts/reviewer/execute
interface ExecuteReviewerRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteReviewerResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `prompts:reviewer:update`
- **Listen Event**: `prompts:reviewer:subscribe`
...
```typescript
// Operational Algorithm for Reviewer Domain Processing
export async function executeReviewerTask(
  ctx: RequestContext,
  payload: ExecuteReviewerRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteReviewerResponseDTO> {
  ...
  const response: ExecuteReviewerResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `REVIEWER_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `REVIEWER_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `REVIEWER_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
