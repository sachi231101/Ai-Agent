...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `frontend`.
...
flowchart TD
    ...
    Gateway --> Service[Frontend Service]
...
## Internal Components
- **`FrontendController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`FrontendService`**: Core business domain engine executing operational logic.
- **`FrontendRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useFrontendStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/prompts/frontend/execute
interface ExecuteFrontendRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteFrontendResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `prompts:frontend:update`
- **Listen Event**: `prompts:frontend:subscribe`
...
```typescript
// Operational Algorithm for Frontend Domain Processing
export async function executeFrontendTask(
  ctx: RequestContext,
  payload: ExecuteFrontendRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteFrontendResponseDTO> {
  ...
  const response: ExecuteFrontendResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `FRONTEND_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `FRONTEND_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `FRONTEND_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
