...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `generator`.
...
flowchart TD
    ...
    Gateway --> Service[Generator Service]
...
## Internal Components
- **`GeneratorController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`GeneratorService`**: Core business domain engine executing operational logic.
- **`GeneratorRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useGeneratorStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/prompts/generator/execute
interface ExecuteGeneratorRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteGeneratorResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `prompts:generator:update`
- **Listen Event**: `prompts:generator:subscribe`
...
```typescript
// Operational Algorithm for Generator Domain Processing
export async function executeGeneratorTask(
  ctx: RequestContext,
  payload: ExecuteGeneratorRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteGeneratorResponseDTO> {
  ...
  const response: ExecuteGeneratorResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `GENERATOR_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `GENERATOR_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `GENERATOR_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
