...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `integration-patterns`.
...
flowchart TD
    ...
    Gateway --> Service[IntegrationPatterns Service]
...
## Internal Components
- **`IntegrationPatternsController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`IntegrationPatternsService`**: Core business domain engine executing operational logic.
- **`IntegrationPatternsRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useIntegrationPatternsStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/reference/integration-patterns/execute
interface ExecuteIntegrationPatternsRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteIntegrationPatternsResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `reference:integration-patterns:update`
- **Listen Event**: `reference:integration-patterns:subscribe`
...
```typescript
// Operational Algorithm for Integration Patterns Domain Processing
export async function executeIntegrationPatternsTask(
  ctx: RequestContext,
  payload: ExecuteIntegrationPatternsRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteIntegrationPatternsResponseDTO> {
  ...
  const response: ExecuteIntegrationPatternsResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `INTEGRATION-PATTERNS_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `INTEGRATION-PATTERNS_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `INTEGRATION-PATTERNS_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
