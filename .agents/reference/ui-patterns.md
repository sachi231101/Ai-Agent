...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `ui-patterns`.
...
flowchart TD
    ...
    Gateway --> Service[UiPatterns Service]
...
## Internal Components
- **`UiPatternsController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`UiPatternsService`**: Core business domain engine executing operational logic.
- **`UiPatternsRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useUiPatternsStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/reference/ui-patterns/execute
interface ExecuteUiPatternsRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteUiPatternsResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `reference:ui-patterns:update`
- **Listen Event**: `reference:ui-patterns:subscribe`
...
```typescript
// Operational Algorithm for Ui Patterns Domain Processing
export async function executeUiPatternsTask(
  ctx: RequestContext,
  payload: ExecuteUiPatternsRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteUiPatternsResponseDTO> {
  ...
  const response: ExecuteUiPatternsResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `UI-PATTERNS_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `UI-PATTERNS_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `UI-PATTERNS_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
