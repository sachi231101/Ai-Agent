...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `backend-patterns`.
...
flowchart TD
    ...
    Gateway --> Service[BackendPatterns Service]
...
## Internal Components
- **`BackendPatternsController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`BackendPatternsService`**: Core business domain engine executing operational logic.
- **`BackendPatternsRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useBackendPatternsStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/reference/backend-patterns/execute
interface ExecuteBackendPatternsRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteBackendPatternsResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `reference:backend-patterns:update`
- **Listen Event**: `reference:backend-patterns:subscribe`
...
```typescript
// Operational Algorithm for Backend Patterns Domain Processing
export async function executeBackendPatternsTask(
  ctx: RequestContext,
  payload: ExecuteBackendPatternsRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteBackendPatternsResponseDTO> {
  ...
  const response: ExecuteBackendPatternsResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `BACKEND-PATTERNS_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `BACKEND-PATTERNS_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `BACKEND-PATTERNS_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
