...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `ai-patterns`.
...
flowchart TD
    ...
    Gateway --> Service[AiPatterns Service]
...
## Internal Components
- **`AiPatternsController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`AiPatternsService`**: Core business domain engine executing operational logic.
- **`AiPatternsRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useAiPatternsStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/reference/ai-patterns/execute
interface ExecuteAiPatternsRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteAiPatternsResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `reference:ai-patterns:update`
- **Listen Event**: `reference:ai-patterns:subscribe`
...
```typescript
// Operational Algorithm for Ai Patterns Domain Processing
export async function executeAiPatternsTask(
  ctx: RequestContext,
  payload: ExecuteAiPatternsRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteAiPatternsResponseDTO> {
  ...
  const response: ExecuteAiPatternsResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `AI-PATTERNS_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `AI-PATTERNS_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `AI-PATTERNS_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
