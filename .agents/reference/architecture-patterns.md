...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `architecture-patterns`.
...
flowchart TD
    ...
    Gateway --> Service[ArchitecturePatterns Service]
...
## Internal Components
- **`ArchitecturePatternsController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`ArchitecturePatternsService`**: Core business domain engine executing operational logic.
- **`ArchitecturePatternsRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useArchitecturePatternsStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/reference/architecture-patterns/execute
interface ExecuteArchitecturePatternsRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteArchitecturePatternsResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `reference:architecture-patterns:update`
- **Listen Event**: `reference:architecture-patterns:subscribe`
...
```typescript
// Operational Algorithm for Architecture Patterns Domain Processing
export async function executeArchitecturePatternsTask(
  ctx: RequestContext,
  payload: ExecuteArchitecturePatternsRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteArchitecturePatternsResponseDTO> {
  ...
  const response: ExecuteArchitecturePatternsResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `ARCHITECTURE-PATTERNS_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `ARCHITECTURE-PATTERNS_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `ARCHITECTURE-PATTERNS_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
