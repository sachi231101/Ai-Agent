...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `node-patterns`.
...
flowchart TD
    ...
    Gateway --> Service[NodePatterns Service]
...
## Internal Components
- **`NodePatternsController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`NodePatternsService`**: Core business domain engine executing operational logic.
- **`NodePatternsRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useNodePatternsStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/reference/node-patterns/execute
interface ExecuteNodePatternsRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteNodePatternsResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `reference:node-patterns:update`
- **Listen Event**: `reference:node-patterns:subscribe`
...
```typescript
// Operational Algorithm for Node Patterns Domain Processing
export async function executeNodePatternsTask(
  ctx: RequestContext,
  payload: ExecuteNodePatternsRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteNodePatternsResponseDTO> {
  ...
  const response: ExecuteNodePatternsResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `NODE-PATTERNS_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `NODE-PATTERNS_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `NODE-PATTERNS_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
