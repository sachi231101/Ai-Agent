...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `troubleshooting`.
...
flowchart TD
    ...
    Gateway --> Service[Troubleshooting Service]
...
## Internal Components
- **`TroubleshootingController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`TroubleshootingService`**: Core business domain engine executing operational logic.
- **`TroubleshootingRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useTroubleshootingStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/reference/troubleshooting/execute
interface ExecuteTroubleshootingRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteTroubleshootingResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `reference:troubleshooting:update`
- **Listen Event**: `reference:troubleshooting:subscribe`
...
```typescript
// Operational Algorithm for Troubleshooting Domain Processing
export async function executeTroubleshootingTask(
  ctx: RequestContext,
  payload: ExecuteTroubleshootingRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteTroubleshootingResponseDTO> {
  ...
  const response: ExecuteTroubleshootingResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `TROUBLESHOOTING_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `TROUBLESHOOTING_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `TROUBLESHOOTING_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
