...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `README`.
...
flowchart TD
    ...
    Gateway --> Service[README Service]
...
## Internal Components
- **`READMEController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`READMEService`**: Core business domain engine executing operational logic.
- **`READMERepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useREADMEStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/examples/README/execute
interface ExecuteREADMERequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteREADMEResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `examples:README:update`
- **Listen Event**: `examples:README:subscribe`
...
```typescript
// Operational Algorithm for README Domain Processing
export async function executeREADMETask(
  ctx: RequestContext,
  payload: ExecuteREADMERequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteREADMEResponseDTO> {
  ...
  const response: ExecuteREADMEResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `README_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `README_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `README_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
