...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `getting-started`.
...
flowchart TD
    ...
    Gateway --> Service[GettingStarted Service]
...
## Internal Components
- **`GettingStartedController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`GettingStartedService`**: Core business domain engine executing operational logic.
- **`GettingStartedRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useGettingStartedStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/docs/getting-started/execute
interface ExecuteGettingStartedRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteGettingStartedResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `docs:getting-started:update`
- **Listen Event**: `docs:getting-started:subscribe`
...
```typescript
// Operational Algorithm for Getting Started Domain Processing
export async function executeGettingStartedTask(
  ctx: RequestContext,
  payload: ExecuteGettingStartedRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteGettingStartedResponseDTO> {
  ...
  const response: ExecuteGettingStartedResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `GETTING-STARTED_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `GETTING-STARTED_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `GETTING-STARTED_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
