...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `release-process`.
...
flowchart TD
    ...
    Gateway --> Service[ReleaseProcess Service]
...
## Internal Components
- **`ReleaseProcessController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`ReleaseProcessService`**: Core business domain engine executing operational logic.
- **`ReleaseProcessRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useReleaseProcessStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/docs/release-process/execute
interface ExecuteReleaseProcessRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteReleaseProcessResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `docs:release-process:update`
- **Listen Event**: `docs:release-process:subscribe`
...
```typescript
// Operational Algorithm for Release Process Domain Processing
export async function executeReleaseProcessTask(
  ctx: RequestContext,
  payload: ExecuteReleaseProcessRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteReleaseProcessResponseDTO> {
  ...
  const response: ExecuteReleaseProcessResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `RELEASE-PROCESS_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `RELEASE-PROCESS_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `RELEASE-PROCESS_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
