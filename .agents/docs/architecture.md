...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `architecture`.
...
flowchart TD
    ...
    Gateway --> Service[Architecture Service]
...
## Internal Components
- **`ArchitectureController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`ArchitectureService`**: Core business domain engine executing operational logic.
- **`ArchitectureRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useArchitectureStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/docs/architecture/execute
interface ExecuteArchitectureRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteArchitectureResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `docs:architecture:update`
- **Listen Event**: `docs:architecture:subscribe`
...
```typescript
// Operational Algorithm for Architecture Domain Processing
export async function executeArchitectureTask(
  ctx: RequestContext,
  payload: ExecuteArchitectureRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteArchitectureResponseDTO> {
  ...
  const response: ExecuteArchitectureResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `ARCHITECTURE_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `ARCHITECTURE_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `ARCHITECTURE_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
