...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `developer-guide`.
...
flowchart TD
    ...
    Gateway --> Service[DeveloperGuide Service]
...
## Internal Components
- **`DeveloperGuideController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`DeveloperGuideService`**: Core business domain engine executing operational logic.
- **`DeveloperGuideRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useDeveloperGuideStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/docs/developer-guide/execute
interface ExecuteDeveloperGuideRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteDeveloperGuideResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `docs:developer-guide:update`
- **Listen Event**: `docs:developer-guide:subscribe`
...
```typescript
// Operational Algorithm for Developer Guide Domain Processing
export async function executeDeveloperGuideTask(
  ctx: RequestContext,
  payload: ExecuteDeveloperGuideRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteDeveloperGuideResponseDTO> {
  ...
  const response: ExecuteDeveloperGuideResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `DEVELOPER-GUIDE_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `DEVELOPER-GUIDE_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `DEVELOPER-GUIDE_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
