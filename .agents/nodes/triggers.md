...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `triggers`.
...
flowchart TD
    ...
    Gateway --> Service[Triggers Service]
...
## Internal Components
- **`TriggersController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`TriggersService`**: Core business domain engine executing operational logic.
- **`TriggersRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useTriggersStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/nodes/triggers/execute
interface ExecuteTriggersRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteTriggersResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `nodes:triggers:update`
- **Listen Event**: `nodes:triggers:subscribe`
...
```typescript
// Operational Algorithm for Triggers Domain Processing
export async function executeTriggersTask(
  ctx: RequestContext,
  payload: ExecuteTriggersRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteTriggersResponseDTO> {
  ...
  const response: ExecuteTriggersResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `TRIGGERS_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `TRIGGERS_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `TRIGGERS_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
