...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `loop-nodes`.
...
flowchart TD
    ...
    Gateway --> Service[LoopNodes Service]
...
## Internal Components
- **`LoopNodesController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`LoopNodesService`**: Core business domain engine executing operational logic.
- **`LoopNodesRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useLoopNodesStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/nodes/loop-nodes/execute
interface ExecuteLoopNodesRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteLoopNodesResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `nodes:loop-nodes:update`
- **Listen Event**: `nodes:loop-nodes:subscribe`
...
```typescript
// Operational Algorithm for Loop Nodes Domain Processing
export async function executeLoopNodesTask(
  ctx: RequestContext,
  payload: ExecuteLoopNodesRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteLoopNodesResponseDTO> {
  ...
  const response: ExecuteLoopNodesResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `LOOP-NODES_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `LOOP-NODES_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `LOOP-NODES_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
