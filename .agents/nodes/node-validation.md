...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `node-validation`.
...
flowchart TD
    ...
    Gateway --> Service[NodeValidation Service]
...
## Internal Components
- **`NodeValidationController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`NodeValidationService`**: Core business domain engine executing operational logic.
- **`NodeValidationRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useNodeValidationStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/nodes/node-validation/execute
interface ExecuteNodeValidationRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteNodeValidationResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `nodes:node-validation:update`
- **Listen Event**: `nodes:node-validation:subscribe`
...
```typescript
// Operational Algorithm for Node Validation Domain Processing
export async function executeNodeValidationTask(
  ctx: RequestContext,
  payload: ExecuteNodeValidationRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteNodeValidationResponseDTO> {
  ...
  const response: ExecuteNodeValidationResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `NODE-VALIDATION_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `NODE-VALIDATION_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `NODE-VALIDATION_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
