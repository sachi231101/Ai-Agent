...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `node-schema`.
...
flowchart TD
    ...
    Gateway --> Service[NodeSchema Service]
...
## Internal Components
- **`NodeSchemaController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`NodeSchemaService`**: Core business domain engine executing operational logic.
- **`NodeSchemaRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useNodeSchemaStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/nodes/node-schema/execute
interface ExecuteNodeSchemaRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteNodeSchemaResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `nodes:node-schema:update`
- **Listen Event**: `nodes:node-schema:subscribe`
...
```typescript
// Operational Algorithm for Node Schema Domain Processing
export async function executeNodeSchemaTask(
  ctx: RequestContext,
  payload: ExecuteNodeSchemaRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteNodeSchemaResponseDTO> {
  ...
  const response: ExecuteNodeSchemaResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `NODE-SCHEMA_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `NODE-SCHEMA_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `NODE-SCHEMA_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
