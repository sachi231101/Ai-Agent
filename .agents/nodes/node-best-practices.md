...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `node-best-practices`.
...
flowchart TD
    ...
    Gateway --> Service[NodeBestPractices Service]
...
## Internal Components
- **`NodeBestPracticesController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`NodeBestPracticesService`**: Core business domain engine executing operational logic.
- **`NodeBestPracticesRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useNodeBestPracticesStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/nodes/node-best-practices/execute
interface ExecuteNodeBestPracticesRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteNodeBestPracticesResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `nodes:node-best-practices:update`
- **Listen Event**: `nodes:node-best-practices:subscribe`
...
```typescript
// Operational Algorithm for Node Best Practices Domain Processing
export async function executeNodeBestPracticesTask(
  ctx: RequestContext,
  payload: ExecuteNodeBestPracticesRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteNodeBestPracticesResponseDTO> {
  ...
  const response: ExecuteNodeBestPracticesResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `NODE-BEST-PRACTICES_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `NODE-BEST-PRACTICES_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `NODE-BEST-PRACTICES_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
...
## Related Files
- [node-system.md](file:///C:/Users/admin/OneDrive/Desktop/emergent/Ai Agent/.agents/nodes/node-system.md)
- [node-schema.md](file:///C:/Users/admin/OneDrive/Desktop/emergent/Ai Agent/.agents/nodes/node-schema.md)
- [ai-nodes.md](file:///C:/Users/admin/OneDrive/Desktop/emergent/Ai Agent/.agents/nodes/ai-nodes.md)
