...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `ai-nodes`.
...
flowchart TD
    ...
    Gateway --> Service[AiNodes Service]
...
## Internal Components
- **`AiNodesController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`AiNodesService`**: Core business domain engine executing operational logic.
- **`AiNodesRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useAiNodesStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/nodes/ai-nodes/execute
interface ExecuteAiNodesRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteAiNodesResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `nodes:ai-nodes:update`
- **Listen Event**: `nodes:ai-nodes:subscribe`
...
```typescript
// Operational Algorithm for Ai Nodes Domain Processing
export async function executeAiNodesTask(
  ctx: RequestContext,
  payload: ExecuteAiNodesRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteAiNodesResponseDTO> {
  ...
  const response: ExecuteAiNodesResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `AI-NODES_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `AI-NODES_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `AI-NODES_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
