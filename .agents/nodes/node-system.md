...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `node-system`.
...
flowchart TD
    ...
    Gateway --> Service[NodeSystem Service]
...
## Internal Components
- **`NodeSystemController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`NodeSystemService`**: Core business domain engine executing operational logic.
- **`NodeSystemRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useNodeSystemStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/nodes/node-system/execute
interface ExecuteNodeSystemRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteNodeSystemResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `nodes:node-system:update`
- **Listen Event**: `nodes:node-system:subscribe`
...
```typescript
// Operational Algorithm for Node System Domain Processing
export async function executeNodeSystemTask(
  ctx: RequestContext,
  payload: ExecuteNodeSystemRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteNodeSystemResponseDTO> {
  ...
  const response: ExecuteNodeSystemResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `NODE-SYSTEM_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `NODE-SYSTEM_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `NODE-SYSTEM_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
