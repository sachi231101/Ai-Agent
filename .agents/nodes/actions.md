...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `actions`.
...
flowchart TD
    ...
    Gateway --> Service[Actions Service]
...
## Internal Components
- **`ActionsController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`ActionsService`**: Core business domain engine executing operational logic.
- **`ActionsRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useActionsStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/nodes/actions/execute
interface ExecuteActionsRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteActionsResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `nodes:actions:update`
- **Listen Event**: `nodes:actions:subscribe`
...
```typescript
// Operational Algorithm for Actions Domain Processing
export async function executeActionsTask(
  ctx: RequestContext,
  payload: ExecuteActionsRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteActionsResponseDTO> {
  ...
  const response: ExecuteActionsResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `ACTIONS_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `ACTIONS_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `ACTIONS_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
