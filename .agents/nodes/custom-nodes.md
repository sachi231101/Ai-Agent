...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `custom-nodes`.
...
flowchart TD
    ...
    Gateway --> Service[CustomNodes Service]
...
## Internal Components
- **`CustomNodesController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`CustomNodesService`**: Core business domain engine executing operational logic.
- **`CustomNodesRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useCustomNodesStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/nodes/custom-nodes/execute
interface ExecuteCustomNodesRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteCustomNodesResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `nodes:custom-nodes:update`
- **Listen Event**: `nodes:custom-nodes:subscribe`
...
```typescript
// Operational Algorithm for Custom Nodes Domain Processing
export async function executeCustomNodesTask(
  ctx: RequestContext,
  payload: ExecuteCustomNodesRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteCustomNodesResponseDTO> {
  ...
  const response: ExecuteCustomNodesResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `CUSTOM-NODES_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `CUSTOM-NODES_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `CUSTOM-NODES_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
