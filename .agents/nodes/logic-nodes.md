...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `logic-nodes`.
...
flowchart TD
    ...
    Gateway --> Service[LogicNodes Service]
...
## Internal Components
- **`LogicNodesController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`LogicNodesService`**: Core business domain engine executing operational logic.
- **`LogicNodesRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useLogicNodesStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/nodes/logic-nodes/execute
interface ExecuteLogicNodesRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteLogicNodesResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `nodes:logic-nodes:update`
- **Listen Event**: `nodes:logic-nodes:subscribe`
...
```typescript
// Operational Algorithm for Logic Nodes Domain Processing
export async function executeLogicNodesTask(
  ctx: RequestContext,
  payload: ExecuteLogicNodesRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteLogicNodesResponseDTO> {
  ...
  const response: ExecuteLogicNodesResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `LOGIC-NODES_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `LOGIC-NODES_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `LOGIC-NODES_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
