...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `workflow-patterns`.
...
flowchart TD
    ...
    Gateway --> Service[WorkflowPatterns Service]
...
## Internal Components
- **`WorkflowPatternsController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`WorkflowPatternsService`**: Core business domain engine executing operational logic.
- **`WorkflowPatternsRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useWorkflowPatternsStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/workflow/workflow-patterns/execute
interface ExecuteWorkflowPatternsRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteWorkflowPatternsResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `workflow:workflow-patterns:update`
- **Listen Event**: `workflow:workflow-patterns:subscribe`
...
```typescript
// Operational Algorithm for Workflow Patterns Domain Processing
export async function executeWorkflowPatternsTask(
  ctx: RequestContext,
  payload: ExecuteWorkflowPatternsRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteWorkflowPatternsResponseDTO> {
  ...
  const response: ExecuteWorkflowPatternsResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `WORKFLOW-PATTERNS_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `WORKFLOW-PATTERNS_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `WORKFLOW-PATTERNS_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
