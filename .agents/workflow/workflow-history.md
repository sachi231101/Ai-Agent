...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `workflow-history`.
...
flowchart TD
    ...
    Gateway --> Service[WorkflowHistory Service]
...
## Internal Components
- **`WorkflowHistoryController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`WorkflowHistoryService`**: Core business domain engine executing operational logic.
- **`WorkflowHistoryRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useWorkflowHistoryStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/workflow/workflow-history/execute
interface ExecuteWorkflowHistoryRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteWorkflowHistoryResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `workflow:workflow-history:update`
- **Listen Event**: `workflow:workflow-history:subscribe`
...
```typescript
// Operational Algorithm for Workflow History Domain Processing
export async function executeWorkflowHistoryTask(
  ctx: RequestContext,
  payload: ExecuteWorkflowHistoryRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteWorkflowHistoryResponseDTO> {
  ...
  const response: ExecuteWorkflowHistoryResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `WORKFLOW-HISTORY_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `WORKFLOW-HISTORY_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `WORKFLOW-HISTORY_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
