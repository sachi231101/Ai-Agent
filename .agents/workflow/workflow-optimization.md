...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `workflow-optimization`.
...
flowchart TD
    ...
    Gateway --> Service[WorkflowOptimization Service]
...
## Internal Components
- **`WorkflowOptimizationController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`WorkflowOptimizationService`**: Core business domain engine executing operational logic.
- **`WorkflowOptimizationRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useWorkflowOptimizationStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/workflow/workflow-optimization/execute
interface ExecuteWorkflowOptimizationRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteWorkflowOptimizationResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `workflow:workflow-optimization:update`
- **Listen Event**: `workflow:workflow-optimization:subscribe`
...
```typescript
// Operational Algorithm for Workflow Optimization Domain Processing
export async function executeWorkflowOptimizationTask(
  ctx: RequestContext,
  payload: ExecuteWorkflowOptimizationRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteWorkflowOptimizationResponseDTO> {
  ...
  const response: ExecuteWorkflowOptimizationResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `WORKFLOW-OPTIMIZATION_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `WORKFLOW-OPTIMIZATION_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `WORKFLOW-OPTIMIZATION_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
