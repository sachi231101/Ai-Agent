...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `workflow-validation`.
...
flowchart TD
    ...
    Gateway --> Service[WorkflowValidation Service]
...
## Internal Components
- **`WorkflowValidationController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`WorkflowValidationService`**: Core business domain engine executing operational logic.
- **`WorkflowValidationRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useWorkflowValidationStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/workflow/workflow-validation/execute
interface ExecuteWorkflowValidationRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteWorkflowValidationResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `workflow:workflow-validation:update`
- **Listen Event**: `workflow:workflow-validation:subscribe`
...
```typescript
// Operational Algorithm for Workflow Validation Domain Processing
export async function executeWorkflowValidationTask(
  ctx: RequestContext,
  payload: ExecuteWorkflowValidationRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteWorkflowValidationResponseDTO> {
  ...
  const response: ExecuteWorkflowValidationResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `WORKFLOW-VALIDATION_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `WORKFLOW-VALIDATION_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `WORKFLOW-VALIDATION_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
