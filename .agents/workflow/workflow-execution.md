...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `workflow-execution`.
...
flowchart TD
    ...
    Gateway --> Service[WorkflowExecution Service]
...
## Internal Components
- **`WorkflowExecutionController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`WorkflowExecutionService`**: Core business domain engine executing operational logic.
- **`WorkflowExecutionRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useWorkflowExecutionStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/workflow/workflow-execution/execute
interface ExecuteWorkflowExecutionRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteWorkflowExecutionResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `workflow:workflow-execution:update`
- **Listen Event**: `workflow:workflow-execution:subscribe`
...
```typescript
// Operational Algorithm for Workflow Execution Domain Processing
export async function executeWorkflowExecutionTask(
  ctx: RequestContext,
  payload: ExecuteWorkflowExecutionRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteWorkflowExecutionResponseDTO> {
  ...
  const response: ExecuteWorkflowExecutionResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `WORKFLOW-EXECUTION_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `WORKFLOW-EXECUTION_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `WORKFLOW-EXECUTION_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
