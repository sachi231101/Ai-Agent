...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `workflow-generation`.
...
flowchart TD
    ...
    Gateway --> Service[WorkflowGeneration Service]
...
## Internal Components
- **`WorkflowGenerationController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`WorkflowGenerationService`**: Core business domain engine executing operational logic.
- **`WorkflowGenerationRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useWorkflowGenerationStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/workflow/workflow-generation/execute
interface ExecuteWorkflowGenerationRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteWorkflowGenerationResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `workflow:workflow-generation:update`
- **Listen Event**: `workflow:workflow-generation:subscribe`
...
```typescript
// Operational Algorithm for Workflow Generation Domain Processing
export async function executeWorkflowGenerationTask(
  ctx: RequestContext,
  payload: ExecuteWorkflowGenerationRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteWorkflowGenerationResponseDTO> {
  ...
  const response: ExecuteWorkflowGenerationResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `WORKFLOW-GENERATION_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `WORKFLOW-GENERATION_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `WORKFLOW-GENERATION_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
...
## Related Files
- [workflow-overview.md](file:///C:/Users/admin/OneDrive/Desktop/emergent/Ai Agent/.agents/workflow/workflow-overview.md)
- [workflow-execution.md](file:///C:/Users/admin/OneDrive/Desktop/emergent/Ai Agent/.agents/workflow/workflow-execution.md)
- [workflow-json.md](file:///C:/Users/admin/OneDrive/Desktop/emergent/Ai Agent/.agents/workflow/workflow-json.md)
- [workflow-validation.md](file:///C:/Users/admin/OneDrive/Desktop/emergent/Ai Agent/.agents/workflow/workflow-validation.md)
