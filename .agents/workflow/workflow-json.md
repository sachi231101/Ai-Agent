...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `workflow-json`.
...
flowchart TD
    ...
    Gateway --> Service[WorkflowJson Service]
...
## Internal Components
- **`WorkflowJsonController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`WorkflowJsonService`**: Core business domain engine executing operational logic.
- **`WorkflowJsonRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useWorkflowJsonStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/workflow/workflow-json/execute
interface ExecuteWorkflowJsonRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteWorkflowJsonResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `workflow:workflow-json:update`
- **Listen Event**: `workflow:workflow-json:subscribe`
...
```typescript
// Operational Algorithm for Workflow Json Domain Processing
export async function executeWorkflowJsonTask(
  ctx: RequestContext,
  payload: ExecuteWorkflowJsonRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteWorkflowJsonResponseDTO> {
  ...
  const response: ExecuteWorkflowJsonResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `WORKFLOW-JSON_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `WORKFLOW-JSON_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `WORKFLOW-JSON_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
