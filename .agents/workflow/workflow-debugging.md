...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `workflow-debugging`.
...
flowchart TD
    ...
    Gateway --> Service[WorkflowDebugging Service]
...
## Internal Components
- **`WorkflowDebuggingController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`WorkflowDebuggingService`**: Core business domain engine executing operational logic.
- **`WorkflowDebuggingRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useWorkflowDebuggingStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/workflow/workflow-debugging/execute
interface ExecuteWorkflowDebuggingRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteWorkflowDebuggingResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `workflow:workflow-debugging:update`
- **Listen Event**: `workflow:workflow-debugging:subscribe`
...
```typescript
// Operational Algorithm for Workflow Debugging Domain Processing
export async function executeWorkflowDebuggingTask(
  ctx: RequestContext,
  payload: ExecuteWorkflowDebuggingRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteWorkflowDebuggingResponseDTO> {
  ...
  const response: ExecuteWorkflowDebuggingResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `WORKFLOW-DEBUGGING_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `WORKFLOW-DEBUGGING_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `WORKFLOW-DEBUGGING_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
