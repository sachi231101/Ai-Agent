...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `workflow-overview`.
...
flowchart TD
    ...
    Gateway --> Service[WorkflowOverview Service]
...
## Internal Components
- **`WorkflowOverviewController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`WorkflowOverviewService`**: Core business domain engine executing operational logic.
- **`WorkflowOverviewRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useWorkflowOverviewStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/workflow/workflow-overview/execute
interface ExecuteWorkflowOverviewRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteWorkflowOverviewResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `workflow:workflow-overview:update`
- **Listen Event**: `workflow:workflow-overview:subscribe`
...
```typescript
// Operational Algorithm for Workflow Overview Domain Processing
export async function executeWorkflowOverviewTask(
  ctx: RequestContext,
  payload: ExecuteWorkflowOverviewRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteWorkflowOverviewResponseDTO> {
  ...
  const response: ExecuteWorkflowOverviewResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `WORKFLOW-OVERVIEW_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `WORKFLOW-OVERVIEW_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `WORKFLOW-OVERVIEW_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
