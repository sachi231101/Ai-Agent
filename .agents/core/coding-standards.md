...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `coding-standards`.
...
flowchart TD
    ...
    Gateway --> Service[CodingStandards Service]
...
## Internal Components
- **`CodingStandardsController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`CodingStandardsService`**: Core business domain engine executing operational logic.
- **`CodingStandardsRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useCodingStandardsStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/core/coding-standards/execute
interface ExecuteCodingStandardsRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteCodingStandardsResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `core:coding-standards:update`
- **Listen Event**: `core:coding-standards:subscribe`
...
```typescript
// Operational Algorithm for Coding Standards Domain Processing
export async function executeCodingStandardsTask(
  ctx: RequestContext,
  payload: ExecuteCodingStandardsRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteCodingStandardsResponseDTO> {
  ...
  const response: ExecuteCodingStandardsResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `CODING-STANDARDS_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `CODING-STANDARDS_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `CODING-STANDARDS_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
