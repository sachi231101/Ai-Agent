...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `error-handling`.
...
flowchart TD
    ...
    Gateway --> Service[ErrorHandling Service]
...
## Internal Components
- **`ErrorHandlingController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`ErrorHandlingService`**: Core business domain engine executing operational logic.
- **`ErrorHandlingRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useErrorHandlingStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/core/error-handling/execute
interface ExecuteErrorHandlingRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteErrorHandlingResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `core:error-handling:update`
- **Listen Event**: `core:error-handling:subscribe`
...
```typescript
// Operational Algorithm for Error Handling Domain Processing
export async function executeErrorHandlingTask(
  ctx: RequestContext,
  payload: ExecuteErrorHandlingRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteErrorHandlingResponseDTO> {
  ...
  const response: ExecuteErrorHandlingResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `ERROR-HANDLING_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `ERROR-HANDLING_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `ERROR-HANDLING_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
