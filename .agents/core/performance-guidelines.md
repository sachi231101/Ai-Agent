...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `performance-guidelines`.
...
flowchart TD
    ...
    Gateway --> Service[PerformanceGuidelines Service]
...
## Internal Components
- **`PerformanceGuidelinesController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`PerformanceGuidelinesService`**: Core business domain engine executing operational logic.
- **`PerformanceGuidelinesRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`usePerformanceGuidelinesStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/core/performance-guidelines/execute
interface ExecutePerformanceGuidelinesRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecutePerformanceGuidelinesResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `core:performance-guidelines:update`
- **Listen Event**: `core:performance-guidelines:subscribe`
...
```typescript
// Operational Algorithm for Performance Guidelines Domain Processing
export async function executePerformanceGuidelinesTask(
  ctx: RequestContext,
  payload: ExecutePerformanceGuidelinesRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecutePerformanceGuidelinesResponseDTO> {
  ...
  const response: ExecutePerformanceGuidelinesResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `PERFORMANCE-GUIDELINES_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `PERFORMANCE-GUIDELINES_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `PERFORMANCE-GUIDELINES_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
