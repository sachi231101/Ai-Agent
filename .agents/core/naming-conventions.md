...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `naming-conventions`.
...
flowchart TD
    ...
    Gateway --> Service[NamingConventions Service]
...
## Internal Components
- **`NamingConventionsController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`NamingConventionsService`**: Core business domain engine executing operational logic.
- **`NamingConventionsRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useNamingConventionsStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/core/naming-conventions/execute
interface ExecuteNamingConventionsRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteNamingConventionsResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `core:naming-conventions:update`
- **Listen Event**: `core:naming-conventions:subscribe`
...
```typescript
// Operational Algorithm for Naming Conventions Domain Processing
export async function executeNamingConventionsTask(
  ctx: RequestContext,
  payload: ExecuteNamingConventionsRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteNamingConventionsResponseDTO> {
  ...
  const response: ExecuteNamingConventionsResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `NAMING-CONVENTIONS_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `NAMING-CONVENTIONS_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `NAMING-CONVENTIONS_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
