...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `security-principles`.
...
flowchart TD
    ...
    Gateway --> Service[SecurityPrinciples Service]
...
## Internal Components
- **`SecurityPrinciplesController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`SecurityPrinciplesService`**: Core business domain engine executing operational logic.
- **`SecurityPrinciplesRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useSecurityPrinciplesStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/core/security-principles/execute
interface ExecuteSecurityPrinciplesRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteSecurityPrinciplesResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `core:security-principles:update`
- **Listen Event**: `core:security-principles:subscribe`
...
```typescript
// Operational Algorithm for Security Principles Domain Processing
export async function executeSecurityPrinciplesTask(
  ctx: RequestContext,
  payload: ExecuteSecurityPrinciplesRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteSecurityPrinciplesResponseDTO> {
  ...
  const response: ExecuteSecurityPrinciplesResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `SECURITY-PRINCIPLES_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `SECURITY-PRINCIPLES_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `SECURITY-PRINCIPLES_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
