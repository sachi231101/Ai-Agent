...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `glossary`.
...
flowchart TD
    ...
    Gateway --> Service[Glossary Service]
...
## Internal Components
- **`GlossaryController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`GlossaryService`**: Core business domain engine executing operational logic.
- **`GlossaryRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useGlossaryStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/core/glossary/execute
interface ExecuteGlossaryRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteGlossaryResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `core:glossary:update`
- **Listen Event**: `core:glossary:subscribe`
...
```typescript
// Operational Algorithm for Glossary Domain Processing
export async function executeGlossaryTask(
  ctx: RequestContext,
  payload: ExecuteGlossaryRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteGlossaryResponseDTO> {
  ...
  const response: ExecuteGlossaryResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `GLOSSARY_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `GLOSSARY_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `GLOSSARY_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
