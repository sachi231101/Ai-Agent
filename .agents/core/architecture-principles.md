...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `architecture-principles`.
...
flowchart TD
    ...
    Gateway --> Service[ArchitecturePrinciples Service]
...
## Internal Components
- **`ArchitecturePrinciplesController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`ArchitecturePrinciplesService`**: Core business domain engine executing operational logic.
- **`ArchitecturePrinciplesRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useArchitecturePrinciplesStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/core/architecture-principles/execute
interface ExecuteArchitecturePrinciplesRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteArchitecturePrinciplesResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `core:architecture-principles:update`
- **Listen Event**: `core:architecture-principles:subscribe`
...
```typescript
// Operational Algorithm for Architecture Principles Domain Processing
export async function executeArchitecturePrinciplesTask(
  ctx: RequestContext,
  payload: ExecuteArchitecturePrinciplesRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteArchitecturePrinciplesResponseDTO> {
  ...
  const response: ExecuteArchitecturePrinciplesResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `ARCHITECTURE-PRINCIPLES_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `ARCHITECTURE-PRINCIPLES_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `ARCHITECTURE-PRINCIPLES_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
...
## Related Files
- [architecture-principles.md](file:///C:/Users/admin/OneDrive/Desktop/emergent/Ai Agent/.agents/core/architecture-principles.md)
- [coding-standards.md](file:///C:/Users/admin/OneDrive/Desktop/emergent/Ai Agent/.agents/core/coding-standards.md)
- [security-principles.md](file:///C:/Users/admin/OneDrive/Desktop/emergent/Ai Agent/.agents/core/security-principles.md)
- [engineering-principles.md](file:///C:/Users/admin/OneDrive/Desktop/emergent/Ai Agent/.agents/core/engineering-principles.md)
