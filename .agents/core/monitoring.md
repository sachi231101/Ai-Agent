...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `monitoring`.
...
## Internal Components
- **`MonitoringController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`MonitoringService`**: Core business domain engine executing operational logic.
- **`MonitoringRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useMonitoringStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/core/monitoring/execute
interface ExecuteMonitoringRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteMonitoringResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `core:monitoring:update`
- **Listen Event**: `core:monitoring:subscribe`
...
```typescript
// Operational Algorithm for Monitoring Domain Processing
export async function executeMonitoringTask(
  ctx: RequestContext,
  payload: ExecuteMonitoringRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteMonitoringResponseDTO> {
  ...
  const response: ExecuteMonitoringResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `MONITORING_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `MONITORING_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `MONITORING_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
