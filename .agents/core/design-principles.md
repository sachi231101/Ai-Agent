...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `design-principles`.
...
flowchart TD
    ...
    Gateway --> Service[DesignPrinciples Service]
...
## Internal Components
- **`DesignPrinciplesController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`DesignPrinciplesService`**: Core business domain engine executing operational logic.
- **`DesignPrinciplesRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useDesignPrinciplesStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/core/design-principles/execute
interface ExecuteDesignPrinciplesRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteDesignPrinciplesResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `core:design-principles:update`
- **Listen Event**: `core:design-principles:subscribe`
...
```typescript
// Operational Algorithm for Design Principles Domain Processing
export async function executeDesignPrinciplesTask(
  ctx: RequestContext,
  payload: ExecuteDesignPrinciplesRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteDesignPrinciplesResponseDTO> {
  ...
  const response: ExecuteDesignPrinciplesResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `DESIGN-PRINCIPLES_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `DESIGN-PRINCIPLES_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `DESIGN-PRINCIPLES_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
