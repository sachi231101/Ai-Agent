...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `deployment`.
...
flowchart TD
    ...
    Gateway --> Service[Deployment Service]
...
## Internal Components
- **`DeploymentController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`DeploymentService`**: Core business domain engine executing operational logic.
- **`DeploymentRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useDeploymentStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/core/deployment/execute
interface ExecuteDeploymentRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteDeploymentResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `core:deployment:update`
- **Listen Event**: `core:deployment:subscribe`
...
```typescript
// Operational Algorithm for Deployment Domain Processing
export async function executeDeploymentTask(
  ctx: RequestContext,
  payload: ExecuteDeploymentRequestDTO,
  prisma: PrismaClient,
  redis: Redis
  ...
  const response: ExecuteDeploymentResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `DEPLOYMENT_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `DEPLOYMENT_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `DEPLOYMENT_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
