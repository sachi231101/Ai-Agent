...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `agent-template`.
...
flowchart TD
    ...
    Gateway --> Service[AgentTemplate Service]
...
## Internal Components
- **`AgentTemplateController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`AgentTemplateService`**: Core business domain engine executing operational logic.
- **`AgentTemplateRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useAgentTemplateStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/templates/agent-template/execute
interface ExecuteAgentTemplateRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteAgentTemplateResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `templates:agent-template:update`
- **Listen Event**: `templates:agent-template:subscribe`
...
```typescript
// Operational Algorithm for Agent Template Domain Processing
export async function executeAgentTemplateTask(
  ctx: RequestContext,
  payload: ExecuteAgentTemplateRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteAgentTemplateResponseDTO> {
  ...
  const response: ExecuteAgentTemplateResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `AGENT-TEMPLATE_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `AGENT-TEMPLATE_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `AGENT-TEMPLATE_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
