...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `integration-template`.
...
flowchart TD
    ...
    Gateway --> Service[IntegrationTemplate Service]
...
## Internal Components
- **`IntegrationTemplateController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`IntegrationTemplateService`**: Core business domain engine executing operational logic.
- **`IntegrationTemplateRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`useIntegrationTemplateStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/templates/integration-template/execute
interface ExecuteIntegrationTemplateRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecuteIntegrationTemplateResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `templates:integration-template:update`
- **Listen Event**: `templates:integration-template:subscribe`
...
```typescript
// Operational Algorithm for Integration Template Domain Processing
export async function executeIntegrationTemplateTask(
  ctx: RequestContext,
  payload: ExecuteIntegrationTemplateRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecuteIntegrationTemplateResponseDTO> {
  ...
  const response: ExecuteIntegrationTemplateResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `INTEGRATION-TEMPLATE_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `INTEGRATION-TEMPLATE_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `INTEGRATION-TEMPLATE_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
