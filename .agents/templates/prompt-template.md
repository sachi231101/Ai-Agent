...
## Responsibilities
- **Domain Logic Ownership**: Manages complete lifecycle, state management, and parameter validation for `prompt-template`.
...
flowchart TD
    ...
    Gateway --> Service[PromptTemplate Service]
...
## Internal Components
- **`PromptTemplateController`**: REST & WebSocket endpoint handlers enforcing JWT authentication and tenant isolation.
- **`PromptTemplateService`**: Core business domain engine executing operational logic.
- **`PromptTemplateRepository`**: Data layer abstraction querying PostgreSQL via Prisma client instance.
- **`usePromptTemplateStore`**: React 19 Zustand hook for reactive canvas state synchronization.
...
### REST API Definition
```typescript
// POST /api/v1/templates/prompt-template/execute
interface ExecutePromptTemplateRequestDTO {
  workspaceId: string;
  agentId?: string;
  workflowId?: string;
...
interface ExecutePromptTemplateResponseDTO {
  success: boolean;
  executionId: string;
...
- **Emit Event**: `templates:prompt-template:update`
- **Listen Event**: `templates:prompt-template:subscribe`
...
```typescript
// Operational Algorithm for Prompt Template Domain Processing
export async function executePromptTemplateTask(
  ctx: RequestContext,
  payload: ExecutePromptTemplateRequestDTO,
  prisma: PrismaClient,
  redis: Redis
): Promise<ExecutePromptTemplateResponseDTO> {
  ...
  const response: ExecutePromptTemplateResponseDTO = {
    success: true,
...
| :--- | :--- | :--- | :--- |
| `PROMPT-TEMPLATE_INVALID_PAYLOAD` | 400 | Zod validation fail | Return structured field error map to caller |
| `PROMPT-TEMPLATE_FORBIDDEN` | 403 | Missing tenant RBAC scope | Reject request and write to Audit Log |
| `PROMPT-TEMPLATE_TIMEOUT` | 504 | Upstream IO / LLM timeout | Trigger automated retry queue job (max 3 retries) |
